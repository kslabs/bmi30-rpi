...
Скрипт выполняет файловую миграцию системы между eMMC и USB:
  ...
  2. По умолчанию переразбивает целевой диск.
  3. С флагом --sync-only использует существующие разделы цели без переразметки.
  4. Копирует boot и root через rsync с прогрессом.
  5. Записывает новые PARTUUID в cmdline.txt и fstab на целевом диске.
...
require_cmds() {
    local missing=()
    local cmd
    for cmd in lsblk findmnt blkid rsync sfdisk wipefs partprobe udevadm mkfs.vfat mkfs.ext4 awk sed grep sync sort; do
...
copy_root_files() {
    ...
    rsync -aHAXx --numeric-ids --delete --human-readable --info=progress2 \
        ...
        --exclude=/lost+found \
        --exclude=/swapfile \
        "$SOURCE_ROOT_COPY_MNT"/ "$TARGET_ROOT_MNT/"
}
