...
prepare_release_manifest() {
    ...
    if [[ -f "$bundle_manifest" ]]; then
        ...
        RELEASE_LABEL="${BMI30_BUNDLE_LABEL:-}"
...
upload_archive() {
    ...
    mkdir -p "$STATE_DIR"
    {
        ...
        printf 'FIRMWARE_LABEL=%q\n' "${RELEASE_LABEL:-}"
        printf 'FIRMWARE_NOTES=%q\n' "${RELEASE_NOTES:-}"
        printf 'BUNDLE_LABEL=%q\n' "${RELEASE_LABEL:-}"
        printf 'BUNDLE_ORIGIN=%q\n' "${RELEASE_NOTES:-}"
        printf 'BUNDLE_CREATED_AT=%q\n' "${RELEASE_BUNDLE_CREATED_AT:-}"
        printf 'DEVICE_SUFFIX=%q\n' "$device_suffix"
        printf 'CREATED_AT=%q\n' "$created_at"
