...
def set_active_button_color(current_oscilloscope, buttons, light_green, text_box):
    if current_oscilloscope == 1:
        ...
        buttons[8].color = light_green
        buttons[8].hovercolor = light_green
        text_box.set_text('Суммы верхней антенны')
    elif current_oscilloscope == 9:
        buttons[9].ax.set_facecolor(light_green)
        buttons[9].color = light_green
        buttons[9].hovercolor = light_green
        text_box.set_text('Суммы нижней антенны')
    else:
...
def update_graph(show_plot, update_needed, current_oscilloscope, start_sample, num_samples, 
    ...
    if show_plot:
        ...
        if current_oscilloscope == 1:
            ...
            line2.set_color('red')
            ax.set_xlim(0, 200)
            ax.set_ylim(-250000, 250000)  # Используем тот же диапазон что и для сумм
            ...
            # Суммы для верхней антенны
            x_axis = np.arange(200)
            line1.set_data(x_axis, sum_P1_1)  # Сумма зоны 1
            line2.set_data(x_axis, sum_P1_2)  # Сумма зоны 2
            line1.set_label('Сумма Зона 1 (365-565)')
            line2.set_label('Сумма Зона 2 (1320-1520)')
            line1.set_color('blue')
            line2.set_color('red')
            ax.set_xlim(0, 200)
            ax.set_ylim(-250000, 250000)  # Расширяем диапазон для сумм (12 * ±20000)
            
        elif current_oscilloscope == 9:
            # Суммы для нижней антенны
            x_axis = np.arange(200)
            line1.set_data(x_axis, sum_L1_1)  # Сумма зоны 1
            line2.set_data(x_axis, sum_L1_2)  # Сумма зоны 2
            line1.set_label('Сумма Зона 1 (350-550)')
            line2.set_label('Сумма Зона 2 (1320-1520)')
            line1.set_color('blue')
            line2.set_color('red')
            ax.set_xlim(0, 200)
