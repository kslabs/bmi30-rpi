...
version = 59.21  # Обновляем версию
...
def save_dc_components():
    ...
    dc_file_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'dc_components.json')
...
def audio_callback(in_data, frame_count, time_info, status):
    ...
    # Добавьте в начале функции проверку активного графика
    # для пропуска ненужных вычислений
    active_graph = current_oscilloscope
    ...
    if len(dataADC_L1) > CHUNK:
        synchronize_data(dataADC_P1)
        dataADC_L1 = dataADC_L1[start_sinchronize:]
        dataADC_P1 = dataADC_P1[start_sinchronize:]
        
        # Вычисления, нужные для всех графиков
        if len(dataADC_P1) > CHUNK and len(dataADC_L1) > CHUNK:
            # Обновление зон только если они нужны для текущего графика
            needs_zones = active_graph in [4, 5, 6, 7, 8, 9, 12, 13, 14, 15]
            if needs_zones:
                ...
                #if not (max_in_range_P1 или max_in_range_L1):
                #    gpio13.off()

            # Выполнение корреляционных вычислений только если
            # они нужны для текущего графика
            needs_correlation = active_graph in [29, 30, 31] or correlation_based_detection
            if needs_correlation:
                # Вычисляем суммы зон с учетом смещений по таймеру
                zone_sum_update_counter += 1
                if zone_sum_update_counter >= ZONE_SUM_UPDATE_INTERVAL or prev_high_sum is None:
                ...
                last_high_hash = 0
                last_low_hash = 0
                last_correlation_result = None

                # Вычисляем хэши текущих данных
                current_high_hash = hash(tuple(sum_high_level_shifted))
                current_low_hash = hash(tuple(sum_low_level_shifted))

                # Используем кэшированный результат, если данные не изменились
                if current_high_hash == last_high_hash and current_low_hash == last_low_hash and last_correlation_result is not None:
                    cross_correlation_sum = last_correlation_result
                else:
                    # Вычисляем кросс-корреляцию только если данные изменились
                    cross_correlation_sum = calculate_cross_correlation(
                        ac_high_level, ac_low_level, CROSS_CORRELATION_SHIFTS)
                    last_correlation_result = cross_correlation_sum
                    last_high_hash = current_high_hash
                    last_low_hash = current_low_hash
        ...
        if current_time - last_dc_save_time >= DC_SAVE_INTERVAL:
            ...
            print(f"++++++++++++++++DC компоненты сохранены: {datetime.now().strftime('%H:%M:%S')}")  # Латинская M
        ...
        if debug_output:
            ...
            if correlation_based_detection and len(cross_correlation_sum) > 0 and len(antiphase_correlation_sum) > 0:
                ...
                if len(correlation_diff) > correlation_index:
                    ...
                    info_str += f" | Корр(+45): {correlation_value:.1f}/{adaptive_correlation_threshold}"
            print(info_str)

        # Сохранение DC составляющих каждые DC_SAVE_INTERVAL секунд
        current_time = time.time()
        if current_time - last_dc_save_time >= DC_SAVE_INTERVAL:
            print(f"*** Сохраняем DC компоненты, прошло {current_time - last_dc_save_time:.1f} секунд")
            # Не используем асинхронное сохранение, чтобы видеть результаты
            success = save_dc_components()
            if success:
                last_dc_save_time = current_time
            else:
                print("Сохранение DC компонент не удалось!")
                # Попробуем снова через минуту
                last_dc_save_time = current_time - DC_SAVE_INTERVAL + 60        
