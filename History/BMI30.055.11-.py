...
prev_cross_correlation = np.zeros(201)  # Инициализация до любого использования
correlation_changes = []  # Инициализируем пустой массив для истории изменений
max_history_length = 100
correlation_change_threshold = 0.3

# глобальные переменные :
reference_correlation_diff = None     # Эталонный шаблон разностной корреляции
reference_update_time = 0             # Время последнего обновления эталона
REFERENCE_UPDATE_INTERVAL = 10        # Интервал обновления эталона (секунды)
pattern_difference_threshold = 0.05    # Порог различия формы (5% вместо 10%)
pattern_detection_count = 0           # Счетчик последовательных обнаружений
pattern_detection_threshold = 2       # Требуемое количество последовательных обнаружений
pattern_detection_enabled = True      # Включить/выключить детекцию по изменению формы

def signal_handler(sig, frame):
    print('Получен сигнал прерывания, завершаем работу...')
    # Здесь код очистки ресурсов
    sys.exit(0)
pattern_signal_active = False  # Флаг активации сигнала
...
version = 55.11  # Обновляем версию
...
def sum_zones_with_shifts_fast(data, zone_marks, zone_length=240, shifts_range=150):
    """
...
correlation_threshold = 65000000000000      # Порог для разницы корреляций
...
# Добавьте следующие глобальные переменные в начало файла
prev_cross_correlation = np.zeros(201)  # Инициализируем до первого использования
correlation_changes = []  # История изменений формы сигнала
max_history_length = 100  # Максимальная длина истории
correlation_change_threshold = 0.3  # Порог изменения формы (30%)

# Функция для расчета меры изменения формы графика
def calculate_waveform_change(current, previous):
    """
    Рассчитывает несколько метрик изменения формы сигнала:
    1. RMSE - среднеквадратичное отклонение
    2. Относительное изменение положения максимума
    3. Относительное изменение значения максимума
    4. Коэффициент корреляции между сигналами
    
    Args:
        current: Текущий сигнал кросс-корреляции
        previous: Предыдущий сигнал кросс-корреляции
        
    Returns:
        Словарь с разными мерами изменения
    """
    if previous is None or len(current) != len(previous):
        return {"rmse": 0, "max_shift": 0, "max_value_change": 0, "correlation": 1.0}
    
    # 1. RMSE - среднеквадратичное отклонение
    rmse = np.sqrt(np.mean((current - previous) ** 2))
    
    # 2. Изменение положения максимума
    current_max_idx = np.argmax(current)
    previous_max_idx = np.argmax(previous)
    max_shift = abs(current_max_idx - previous_max_idx)
    
    # 3. Относительное изменение значения максимума
    current_max_value = np.max(current)
    previous_max_value = np.max(previous)
    
    if previous_max_value != 0:
        max_value_change = abs(current_max_value - previous_max_value) / previous_max_value
    else:
        max_value_change = 0
    
    # 4. Коэффициент корреляции между сигналами
    # Нормализуем сигналы для более точного сравнения формы
    current_norm = (current - np.mean(current)) / (np.std(current) if np.std(current) > 0 else 1)
    previous_norm = (previous - np.mean(previous)) / (np.std(previous) if np.std(previous) > 0 else 1)
...
# Добавьте эту функцию для сравнения форм сигналов:
def compare_signal_patterns(current, reference):
    """
    Сравнивает форму текущего сигнала с эталонным шаблоном
    Возвращает значение от 0 до 1, где 0 - идентичные, 1 - полностью различные
    """
    if reference is None or len(current) != len(reference):
        return 0.0
    
    # Нормализуем сигналы для сравнения только формы (не амплитуды)
    ...
    if current_std == 0 or reference_std == 0:
        return 0.5  # Если один из сигналов константа - считаем средним различием
    
    current_norm = (current - current_mean) / current_std
    reference_norm = (reference - reference_mean) / reference_std
    
    # Вычисляем корреляцию между нормализованными сигналами
    try:
        correlation = np.corrcoef(current_norm, reference_norm)[0, 1]
        # Преобразуем корреляцию в меру различия (0-1)
        # Абсолютное значение корреляции, т.к. нас интересует только форма, а не знак
        difference = 1.0 - abs(correlation)
        return difference
    except:
        return 0.5  # В случае ошибки вычисления возвращаем среднее значение
...
def audio_callback(in_data, frame_count, time_info, status):
    ...
    global antiphase_correlation_sum  # Добавляем глобальную переменную
    global prev_cross_correlation, correlation_changes, max_history_length, correlation_change_threshold
    ...
    if len(dataADC_L1) > CHUNK:
        ...
        if len(dataADC_P1) > CHUNK and len(dataADC_L1) > CHUNK:
            ...
            # Сохраняем предыдущую кросс-корреляцию
            if prev_cross_correlation is not None:
                waveform_change = calculate_waveform_change(cross_correlation_sum, prev_cross_correlation)
                # Сохраняем только числовое значение, а не объект
                change_value = 1 - waveform_change["correlation"]  # Преобразуем корреляцию в меру изменения
                # Проверяем, что значение имеет смысл
                if not np.isnan(change_value) and 0 <= change_value <= 1:
                    correlation_changes.append(change_value)  # Сохраняем только число
                    if len(correlation_changes) > max_history_length:
                        correlation_changes.pop(0)
            prev_cross_correlation = cross_correlation_sum.copy()
...
# Добавьте после кнопки beep_mode_button
ax_detection_button = plt.axes([0.83, 0.94, 0.15, 0.03])
...
# Добавьте эту кнопку в интерфейс для включения/выключения метода обнаружения:
ax_pattern_button = plt.axes([0.6, 0.94, 0.23, 0.03])  # Позиция кнопки
pattern_button = Button(ax_pattern_button, "Детектор изменения формы: ВКЛ", color='lightgreen')

def toggle_pattern_detection(event=None):
    global pattern_detection_enabled
    pattern_detection_enabled = not pattern_detection_enabled
    if pattern_detection_enabled:
        pattern_button.label.set_text("Детектор изменения формы: ВКЛ")
        pattern_button.color = 'lightgreen'
    else:
        pattern_button.label.set_text("Детектор изменения формы: ВЫКЛ")
        pattern_button.color = 'lightcoral'
        # Сбрасываем все связанные счетчики
        pattern_detection_count = 0
        stop_beep()
    print(f"Детектор изменения формы: {'включен' if pattern_detection_enabled else 'выключен'}")
    fig.canvas.draw_idle()

pattern_button.on_clicked(toggle_pattern_detection)
...
try:
    ...
    while True:
        ...
        continue_loop, update_needed = update_graph(
            ...
            ac_high_level,
            ac_low_level,
            # Противофазная корреляция
            antiphase_correlation_sum,
            correlation_threshold,  # Добавляем параметр порога для графика 29
            correlation_detection_count,  # Добавляем счетчик детекций
            correlation_detection_threshold,  # Добавляем порог счетчика
            correlation_changes,
            correlation_change_threshold  # Добавляем порог изменения формы
        )
        if not continue_loop:
            break

        # В основном цикле программы для обработки формы сигнала
        # Всегда вычисляем изменения формы сигнала, независимо от активного графика
        if prev_cross_correlation is not None and len(cross_correlation_sum) == len(prev_cross_correlation):
            # Рассчитываем изменение формы сигнала
            changes = calculate_waveform_change(cross_correlation_sum, prev_cross_correlation)
            
            # Вычисляем значение изменения - чем выше, тем сильнее изменение формы
            change_value = 1 - changes["correlation"]  # Преобразуем корреляцию в меру изменения
            
            # Добавляем в историю только если значение имеет смысл 
            # (проверяем на NaN и нормальный диапазон)
            if not np.isnan(change_value) and 0 <= change_value <= 1:
                correlation_changes.append(change_value)
                print(f"Добавлено изменение формы сигнала: {change_value:.4f}, элементов в истории: {len(correlation_changes)}")
            
            # Ограничиваем длину истории
            while len(correlation_changes) > max_history_length:
                correlation_changes.pop(0)
            
            # Выводим информацию об изменениях только если активен график 31 или 27
            if current_oscilloscope in [27, 31] and show_plot:
                print(f"Изменение сигнала на графике 31:")
                print(f"  - RMSE: {changes['rmse']:.2f}")
                print(f"  - Смещение максимума: {changes['max_shift']} отсчетов")
                print(f"  - Изменение макс. значения: {changes['max_value_change']*100:.1f}%")
                print(f"  - Корреляция форм: {changes['correlation']:.4f}")
                print(f"  - Общее изменение формы: {change_value*100:.1f}%")

        # Сохраняем текущий сигнал для следующего сравнения
        prev_cross_correlation = cross_correlation_sum.copy()

        # В основном цикле, перед проверкой условия обнаружения сигнала:
        ...
        if debug_output:
            ...
            if correlation_based_detection and len(cross_correlation_sum) > 0 and len(antiphase_correlation_sum) > 0:
                ...
                if len(correlation_diff) > correlation_index:
                    ...
                    info_str += f" | Корр(+45): {correlation_value:.1f}/{correlation_threshold}"
            print(info_str)
        ...
        if prev_cross_correlation is not None and len(cross_correlation_sum) == len(prev_cross_correlation):
            ...
            # Сохраняем в историю изменений только величину изменения (число)
            correlation_changes.append(1 - changes["correlation"])  # 1 - correlation: чем больше, тем сильнее изменение
            ...
            if current_oscilloscope in [27, 31] and show_plot:
                ...
                print(f"  - Изменение макс. значения: {changes['max_value_change']*100:.1f}%")
                print(f"  - Корреляция форм: {changes['correlation']:.4f}")
                print(f"  - Общее изменение формы: {(1-changes['correlation'])*100:.1f}%")

        # Сохраняем текущий сигнал для следующего сравнения (делаем это всегда)
        prev_cross_correlation = cross_correlation_sum.copy()

        
        if pattern_detection_enabled and len(cross_correlation_sum) > 0 and len(antiphase_correlation_sum) > 0:
            # Вычисляем разницу корреляций
            correlation_diff = cross_correlation_sum - antiphase_correlation_sum
            
            current_time = time.time()
            
            # Инициализируем эталон, если его ещё нет
            if reference_correlation_diff is None or len(reference_correlation_diff) != len(correlation_diff):
                reference_correlation_diff = correlation_diff.copy()
                reference_update_time = current_time
                print("Эталонная форма разностной корреляции инициализирована")
            
            # Сравниваем текущий сигнал с эталоном
            pattern_difference = compare_signal_patterns(correlation_diff, reference_correlation_diff)
            
            # Логика обнаружения сигнала с гистерезисом
            if not pattern_signal_active:  # Если сигнал не активен
                # Используем верхний порог для активации
                if pattern_difference > pattern_activation_threshold:
                    # Увеличиваем счетчик последовательных обнаружений
                    pattern_detection_count += 1
                    
                    # Если счетчик превысил порог - активируем сигнал
                    if pattern_detection_count >= pattern_detection_threshold:
                        pattern_signal_active = True  # Устанавливаем флаг активности
                        
                        # Сигнал обнаружен - включаем звук
                        if beep_mode == "continuous":
                            start_continuous_beep()
                        else:
                            start_intermittent_beep()
                        ...
                        # Выводим информацию об обнаружении
                        print(f"Сигнал обнаружен по изменению формы! Различие: {pattern_difference:.2f}, Порог: {pattern_activation_threshold:.2f}")
                else:
                    # Сбрасываем счетчик если значение ниже порога
                    pattern_detection_count = 0
            else:  # Если сигнал активен
                # Для деактивации используем нижний порог (с гистерезисом)
                if pattern_difference < pattern_deactivation_threshold:
                    # Сбрасываем счетчик и деактивируем сигнал только если он ниже нижнего порога
                    pattern_detection_count = 0
                    pattern_signal_active = False
                    stop_beep()
                    print(f"Сигнал потерян (различие: {pattern_difference:.2f} < {pattern_deactivation_threshold:.2f})")
            
            # Обновляем эталон только если сигнал не активен и прошло достаточно времени
            if not pattern_signal_active and current_time - reference_update_time > REFERENCE_UPDATE_INTERVAL:
                # Плавное обновление (90% старого шаблона + 10% нового сигнала)
                reference_correlation_diff = 0.9 * reference_correlation_diff + 0.1 * correlation_diff
                reference_update_time = current_time
                if debug_output:
                    print(f"Эталонная форма обновлена (различие: {pattern_difference:.3f})")
            
            # Выводим отладочную информацию
            if debug_output:
                threshold_status = "АКТИВ" if pattern_signal_active else ("ПРЕВЫШЕН" if pattern_difference > pattern_activation_threshold else "норма")
                print(f"Изменение формы графика 29: {pattern_difference:.3f}/{pattern_activation_threshold:.2f} ({threshold_status}) | "
                      f"Счетчик: {pattern_detection_count}/{pattern_detection_threshold} | Сигнал: {'ВКЛ' if pattern_signal_active else 'ВЫКЛ'}")
