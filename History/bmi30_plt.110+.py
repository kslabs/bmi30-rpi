...
def update_graph(show_plot, update_needed, current_oscilloscope, start_sample, num_samples, dataADC_P1, dataADC_L1, line1, line2, ax, fig):
    ...
    if not plt.fignum_exists(fig.number):
        return False, update_needed
    if show_plot:
        # Обновляем данные осциллограммы
        if current_oscilloscope == 1:
            line1.set_ydata(dataADC_P1[start_sample:start_sample+num_samples])
            line2.set_ydata(dataADC_L1[start_sample:start_sample+num_samples])
        elif current_oscilloscope == 2:
            ...
            line2.set_ydata(dataADC_L1[start_sample:start_sample+num_samples])
        if np.any(dataADC_P1[start_sample:start_sample+num_samples]) or np.any(dataADC_L1[start_sample:start_sample+num_samples]):
            fig.canvas.flush_events()
    return True, update_needed
