...
dataADC = np.zeros(MAX_DATA_SIZE, dtype=np.int16)
dataADC1 = np.zeros(MAX_DATA_SIZE, dtype=np.int16)
dataADC2 = np.zeros(MAX_DATA_SIZE, dtype=np.int16)
dataADC3 = np.zeros(MAX_DATA_SIZE, dtype=np.int16)
dataADC4 = np.zeros(MAX_DATA_SIZE, dtype=np.int16)
dataADC5 = np.zeros(MAX_DATA_SIZE, dtype=np.int16)
dataADC6 = np.zeros(MAX_DATA_SIZE, dtype=np.int16)
dataADC7 = np.zeros(MAX_DATA_SIZE, dtype=np.int16)
dataINP1 = np.zeros(CHUNK_out2, dtype=np.int16)       # массив с компенсациЕЙ постоянной составляющей и АРУ
dataINP11 = np.zeros(CHUNK_out2, dtype=np.int16)       # массив с компенсациЕЙ постоянной составляющей 
dataINP_result = np.zeros(CHUNK_out2, dtype=np.int16)                   # Результирующий массив
dataINP_result1 = np.zeros(CHUNK_out2, dtype=np.int16)                   # Результирующий массив
dataINP_res_tau = np.zeros(CHUNK_out2, dtype=np.int16)              # массив длительностей роста и спада 

dataDC = np.zeros(CHUNK_out2, dtype=np.int16)         # массив с постоянной составляющей 
data_to_sum_p = np.zeros(CHUNK_out2, dtype=np.int16)
data_to_sum_n = np.zeros(CHUNK_out2, dtype=np.int16)
temp_add20 = np.zeros(20, dtype=np.int16)
flag_synchro = 0
ADCmax = 0

@njit
def process_data(dataADC1, dataADC2, dataADC3, dataADC4, dataADC5, dataADC6, dataADC7, new_data, start_adr, count_start, start_adress, stop_adress, tau_start_adr, CHUNK, CHUNK0, flag_synchro, ADCmax):
    if len(dataADC1) > CHUNK:
        for i in range(CHUNK):
            ...
            if i > CHUNK0:
                # ??????????? ?????? ? ?????????????? ??????
                dataADC7[:] = dataADC6[:CHUNK]
                dataADC6[:] = dataADC5[:CHUNK]
                dataADC5[:] = dataADC4[:CHUNK]
                dataADC4[:] = dataADC3[:CHUNK]
                dataADC3[:] = dataADC2[:CHUNK]
...
def callback(in_data, frame_count, time_info, status):
    ...
    dataADC1, dataADC2, dataADC3, dataADC4, dataADC5, dataADC6, dataADC7, count_start, data_to_sum_p, data_to_sum_n, flag_synchro, ADCmax = process_data(dataADC1, dataADC2, dataADC3, dataADC4, dataADC5, dataADC6, dataADC7, new_data, start_adr, count_start, start_adress, stop_adress, tau_start_adr, CHUNK, CHUNK0, flag_synchro, ADCmax)
