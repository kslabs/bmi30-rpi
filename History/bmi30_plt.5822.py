...
BUTTON_CONFIG = {
    ...
    3: {
        ...
        'add_markers': [1370, 3252, 5149, 7047, 9047, 10930, 12825, 14723, 16726, 18607, 20504, 22403]  # Метки зон 3255, 3495
    },
    4: {
        'text': 'Зоны ожидания',
        'show_plot': True,
    ...
    if show_plot:
        ...
        if current_oscilloscope == 1:
            ...
            if cross_correlation_sum is not None and antiphase_correlation_sum is not None and len(cross_correlation_sum) > 0 and len(antiphase_correlation_sum) > 0:
                ...
                # Очищаем график и рисуем основную линию
                ax.clear()
                ax.plot(shifts, correlation_diff, 'g-', linewidth=1.5, label='Разница корреляций')
                ...
                # Автоматический расчёт пределов по Y с запасом
                y_max = np.max(np.abs(correlation_diff)) * 1.2
                ...
                # Безопасный доступ к переменным градиентного метода:
                current_distortion_index = globals().get('distortion_index', 0)
                current_gradient_threshold = globals().get('gradient_threshold', 0)
                
                # Использовать переменные только если они определены
                if current_distortion_index > 0:
                    ax.text(0.02, 0.85, f'Индекс искажения: {current_distortion_index:.0f}', 
                            transform=ax.transAxes, fontsize=9,
                            bbox=dict(boxstyle='round', facecolor='lightyellow', alpha=0.6))
                            
                if current_gradient_threshold > 0:
                    ax.text(0.02, 0.80, f'Порог: {current_gradient_threshold:.0f}', 
                            transform=ax.transAxes, fontsize=9,
                            bbox=dict(boxstyle='round', facecolor='lightgreen' 
                                      if current_distortion_index > current_gradient_threshold else 'white', alpha=0.6))
