...
if waveform_output_display:
    ...
    if waveform_output_display == 1:
        ...
        display_line, = ax.plot(x_axis, np.random.rand(CHU10NK_out1), 'r')
...
#    p.terminate()
    ...
    while stream.is_active():
        ...
        if start_adr > 9:
            ...
            if tau_start_adr==0 and dataINP_max > trigger_treshold2 + (int(count_delay_tag / 100) * 10000):
                flag_det_tag += 1
                marked_tau = '+'
            else:
                if tau_start_adr:
            ...
            temp2 = np.diff(dataINP_res_tau, n=2, prepend=0, append=0)
            temp3 = np.diff(temp2, n=2, prepend=0, append=0)
            ...
            if count_time_detect > 10: 
                if flag_det_tag > 0 and abs(mean_dataINP_count) < 20:   # первые 10 сек не детектировать и проверять условие: средняя составляющая должна быть меньше 33. 
                    ...
                    print(marked_tau, f'{count_delay_tag:4}', f'Метка определена с максимальным уровнем сигнала:{dataINP_max:6}', f'| Mean:{mean_dataINP_count:4}', f'/{max_mean:4}','/',tau_start_adr, '      !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!       ', end='\r')
                    ...
                    if count_delay_tag >0:
                        ...
                        if abs(mean_dataINP_count) > abs(max_mean):
                            max_mean = mean_dataINP_count
                    print(marked_tau, f'{count_delay_notag:4}', f'Метки нет.         Максимальный уровень сигнала:{dataINP_max:6}', f'| Mean:{mean_dataINP_count:4}', f'/{max_mean:4}','/', tau_start_adr, '      ----------------------------------------------------------      ', end='\r')
                    count_delay_tag = 0
                    count_delay_notag += 1
