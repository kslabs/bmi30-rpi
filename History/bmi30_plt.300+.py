...
def update_graph(show_plot, update_needed, current_oscilloscope, start_sample, num_samples, 
    ...
    if show_plot:
        ...
        if current_oscilloscope == 1:
            ...
            # Центрируем каждую зону отдельно относительно нуля
            centered_ac_P1_1 = ac_P1_1 - np.mean(ac_P1_1)
            centered_ac_P1_2 = ac_P1_2 - np.mean(ac_P1_2)
            
            # Рисуем центрированные данные
            ax.plot(range(200), centered_ac_P1_1, color='green', label='AC зоны 1')
            ax.plot(range(200), centered_ac_P1_2, color='red', label='AC зоны 2')
            ax.legend()
            ...
            # Центрируем каждую зону отдельно относительно нуля
            centered_ac_L1_1 = ac_L1_1 - np.mean(ac_L1_1)
            centered_ac_L1_2 = ac_L1_2 - np.mean(ac_L1_2)
            
            # Рисуем центрированные данные
            ax.plot(range(200), centered_ac_L1_1, color='green', label='AC зоны 1')
            ax.plot(range(200), centered_ac_L1_2, color='red', label='AC зоны 2')
            ax.legend()
            ...
            try:
                ...
                # Импортируем массивы нормализованных периодов
                from __main__ import periods_P1_1, periods_P1_2
                
                # Отображаем все 12 периодов для обеих зон
                for i in range(12):
                    # Зона 1 (сдвиг вверх)
                    if len(periods_P1_1) > i:
                        # Берем данные из нормализованных массивов
                        data1 = periods_P1_1[i].copy()
                        # Центрируем с полной компенсацией формы
                        if len(dc_P1_1) == 200:
                            data1 = data1 - dc_P1_1  # Вычитаем всю DC-компоненту
                        # Сдвигаем вверх
                        data1 += 1000
                        # Рисуем линию
                    ...
                    # Зона 2 (сдвиг вниз)
                    if len(periods_P1_2) > i:
                        # Берем данные из нормализованных массивов
                        data2 = periods_P1_2[i].copy()
                        # Центрируем с полной компенсацией формы
                        if len(dc_P1_2) == 200:
                            data2 = data2 - dc_P1_2  # Вычитаем всю DC-компоненту
                        # Сдвигаем вниз
                        data2 -= 1000
            ...
            try:
                ...
                # Импортируем массивы нормализованных периодов
                from __main__ import periods_L1_1, periods_L1_2
                
                # Отображаем все 12 периодов для обеих зон нижней антенны
                for i in range(12):
                    # Зона 1 (сдвиг вверх)
                    if len(periods_L1_1) > i:
                        # Берем данные из нормализованных массивов
                        data1 = periods_L1_1[i].copy()
                        # Центрируем с полной компенсацией формы
                        if len(dc_L1_1) == 200:
                            data1 = data1 - dc_L1_1  # Вычитаем всю DC-компоненту
                        # Сдвигаем вверх
                        data1 += 1000
                        # Рисуем линию
                        line, = ax.plot(range(200), data1, color=colors[i], alpha=0.7)
                    ...
                    # Зона 2 (сдвиг вниз)
                    if len(periods_L1_2) > i:
                        # Берем данные из нормализованных массивов
                        data2 = periods_L1_2[i].copy()
                        # Центрируем с полной компенсацией формы
                        if len(dc_L1_2) == 200:
                            data2 = data2 - dc_L1_2  # Вычитаем всю DC-компоненту
                        # Сдвигаем вниз
                        data2 -= 1000
                        # Рисуем пунктирную линию
                        ax.plot(range(200), data2, color=colors[i], alpha=0.7, linestyle='--')
        ...
        elif current_oscilloscope >= 14 and current_oscilloscope <= 15:
