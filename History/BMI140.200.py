...
def callback(in_data, frame_count, time_info, status):
    ...
    global dataADC, dataADC1, dataADC2, dataADC3, dataADC4, dataADC5, dataADC6, dataADC7,dataADC8, dataADC9, new_data
    ...
    # Получаем данные буфера и разделяем на левый и правый каналы
    dataADC1 = np.concatenate((dataADC1, np.frombuffer(in_data, dtype=np.int16)[0::2]))
    #dataADC02 = np.concatenate((dataADC02, np.frombuffer(in_data[1::2], dtype=np.int16)))
    
    #new_data = np.frombuffer(in_data, dtype=np.int16)
    #dataADC1 = np.concatenate((dataADC1, new_data[0::2]))
    
    print(len(dataADC1), dataADC1)
    
    #dataInt = list(struct.unpack(str(CHUNK * CHANNELS) + 'h', data))
    #dataInt1 = dataInt[0::2]  # ЛевыйЙ канал
...
try:                        # для аккуратного принудительного останова программы с закрытием портов
    ...
    while running and stream.is_active():
        ...
        if start_adr > 9:
            ...
            dataINP11 = ((data_to_sum_p - data_to_sum_p) * 500) - dataDC
