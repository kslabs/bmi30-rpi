...
def switch_graph(label, buttons, light_green, text_box, current_oscilloscope, update_needed, show_plot, fig):
    ...
    if label == '0':
        ...
        text_box.set_text('Спектральный анализ (40-1000 Гц)')
        print(f"Переключение на график {current_oscilloscope[0]}")
    elif label == '11':
        current_oscilloscope[0] = int(label)
        show_plot[0] = True
        ...
        text_box.set_text('Спектр разностей (40-1000 Гц)')
        print(f"Переключение на график {current_oscilloscope[0]}")
    elif label == '12':
        current_oscilloscope[0] = int(label)
        show_plot[0] = True
...
def update_graph(show_plot, update_needed, current_oscilloscope, start_sample, num_samples, 
                ...
                zone_P1_1, zone_P1_2, zone_L1_1, zone_L1_2,  # Зоны
                ac_P1_1, ac_P1_2, ac_L1_1, ac_L1_2,          # AC компоненты
                dc_P1_1, dc_P1_2, dc_L1_1, dc_L1_2,          # DC компоненты
                sum_P1_1, sum_P1_2, sum_L1_1, sum_L1_2,      # Суммы
                diff_sum_P1, diff_sum_L1,                     # Разности
                max_sample_index_P1, max_sample_index_L1):    # Индексы максимумов
    if not show_plot:
        return True, False
    
    if show_plot:
        if current_oscilloscope == 1:
            line1.set_data(range(start_sample, start_sample + num_samples),
                          dataADC_P1[start_sample:start_sample + num_samples])
            line2.set_data(range(start_sample, start_sample + num_samples),
                          dataADC_L1[start_sample:start_sample + num_samples])
            line1.set_label('Верхняя антенна')
            line2.set_label('Нижняя антенна')
            line2.set_visible(True)
            ax.legend()
            
        # ... оставляем весь существующий код без изменений ...
            
        elif current_oscilloscope == 12:
            # Очищаем дополнительные линии
            ...
            for i in range(12):
                start_idx = 365 + i * period
                if start_idx + 200 <= len(dataADC_P1):
                    data = dataADC_P1[start_idx:start_idx + 200]
                    data_centered = data - dc_P1_1  # Используем переменную из параметров
                    ax.plot(range(200), data_centered, 
                           color=colors[i], 
                           alpha=0.7,
                           label=f'Период {i+1}')
