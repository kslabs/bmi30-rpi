...
version = 38.00  # Обновляем версию
...
count_phase = 1                 # Счетчик для фазы
stable_phase_threshold = 5      # Порог стабильности фазы (±5 отсчетов)
amplitude_threshold = 80000     # Порог амплитуды для сигнализации
...
threshold_margin = 1.20         # Множитель для порога (10% запас)
...
adaptive_threshold_history = []  # История изменения порога для отладки

threshold_increase_rate = 1    # Скорость увеличения порога при наличии сигнала
below_threshold_counter = 0     # Счетчик измерений ниже порога
below_threshold_limit = 5      # Сколько раз подряд должно быть ниже порога для включения адаптации
...
# Добавьте после блока объявления переменных пользовательского интерфейса
# примерно в районе строки 920
last_terminal_update = time.time()  # Время последнего обновления информации в терминале
terminal_update_interval = 2.0  # Интервал обновления в секундах (раз в 2 секунды)
...
def stop_beep():
    ...
    try:
        ...
        #time.sleep(0.05)
        #os.system(f"echo out > /sys/class/gpio/gpio13/direction 2>/dev/null")
        #time.sleep(0.05)
        #os.system(f"echo 0 > /sys/class/gpio/gpio13/value 2>/dev/null")
...
try:
    ...
    while True:
        ...
        continue_loop, update_needed = update_graph_wrapper(
            show_plot, 
            update_needed, 
            ...
            # Добавляем новые параметры - массивы периодов
            periods_P1_1,
            periods_P1_2,
            periods_L1_1,
            periods_L1_2
        ...
        # Логика для звукового оповещения с проверкой амплитуды и стабильности фазы
        # Добавляем отладочную информацию
        debug_output = 1  # Установите 0=выкл, 1=вкл
        
        # Управление частотой вывода в терминал
        current_time = time.time()
        show_terminal_info = (current_time - last_terminal_update >= terminal_update_interval)
        
        # Выводим основную информацию через заданные интервалы
        if debug_output and show_terminal_info:
            last_terminal_update = current_time
            
            # Собираем информацию для статусной строки
            status_parts = []
            
            # Информация об амплитуде и пороге
            status_parts.append(f"Амплитуда: {max_amp_15:6.0f} | Порог: {adaptive_threshold:6.0f}")
            status_parts.append(f"Фаза: {max_phase_15:3.0f} | Δфазы: {abs(max_phase_15 - prev_max_phase_15):2.0f}")
            status_parts.append(f"Обнаружений: {consecutive_detections}/{count_phase}")
            
            # Информация о времени без сигнала, если нет активного сигнала
            if max_amp_15 <= adaptive_threshold:
                ...
                if last_max_amplitude > 0:
                    status_parts.append(f"Без сигнала: {time_str}")
                    status_parts.append(f"Последний: A={last_max_amplitude:.0f}, Ф={last_max_phase:.0f} | {last_beep_timestamp}")
                else:
                    status_parts.append(f"Без сигнала: {time_str}")
            
            status_parts.append(f"Режим: {'Адаптивный' if threshold_update_enabled else 'Фиксированный'}")
            status_parts.append(f"Звук: {beep_mode}")
            
            # Обновляем глобальную строку статуса
            info_message = " | ".join(status_parts)
            
            # Отображаем текущее состояние
            display_status()

        # Логика адаптивного порога
        if max_amp_15 > adaptive_threshold:
            ...
            if threshold_update_enabled:
                ...
                if debug_output:
                    print(f"Сигнал обнаружен, адаптация отключена, порог: {adaptive_threshold:.0f}")
            
            # Сбрасываем счетчик измерений ниже порога
            below_threshold_counter = 0
            
            #if debug_output:
            #    print(f"Порог постепенно повышается: {adaptive_threshold:.0f}")
        elif max_amp_15 > 0 and max_amp_15 * threshold_margin < adaptive_threshold:
            # Сигнал ниже порога
            below_threshold_counter += 1
            
            if threshold_update_enabled:
                # Адаптация включена - настраиваем порог под уровень шума
                if max_amp_15 > 0:  # Если есть какой-то сигнал
                    # Устанавливаем порог чуть выше уровня шума
                    new_threshold = max_amp_15 * threshold_margin
                    # Если новый порог выше текущего - используем его
                    # иначе медленно снижаем текущий порог
                    if new_threshold > adaptive_threshold:
                        adaptive_threshold = new_threshold
                        if debug_output:
                            print(f"Порог повышен до уровня шума+20%: {adaptive_threshold:.0f}")
                    else:
                        adaptive_threshold -= threshold_decrease_rate
                        #if debug_output и adaptive_threshold % 100 == 0:
                        #    print(f"Порог постепенно снижается: {adaptive_threshold:.0f}")
        ...
        # Условие обнаружения сигнала с адаптивным порогом
        if max_amp_15 > adaptive_threshold:
            # Проверяем, находится ли новый максимум на той же фазе (в пределах порога)
            if abs(max_phase_15 - prev_max_phase_15) <= stable_phase_threshold:
                consecutive_detections += 1
                # Сигнализируем только если максимум стабилен в течение нужного числа отсчетов
                if consecutive_detections >= count_phase:
                    ...
                    last_max_amplitude = max_amp_15
                    last_max_phase = max_phase_15
                    last_phase_diff = abs(max_phase_15 - prev_max_phase_15)
                    last_beep_timestamp = datetime.now().strftime("%H:%M:%S")
                    
                    # При обнаружении сигнала добавляем сообщение в лог вместо прямого вывода
                    signal_message = f"СИГНАЛ ОБНАРУЖЕН! Амплитуда: {max_amp_15:.0f}, Порог: {adaptive_threshold:.0f}, Фаза: {max_phase_15}"
                    add_log_message(signal_message)
                    
                    # Сбрасываем счетчик времени тишины и предупреждения
                    last_beep_time = time.time()
                    silent_warnings_shown = {period: False for period in silent_warning_periods}
            else:
                # Если фаза изменилась значительно, сбрасываем счетчик
                consecutive_detections = 1
                if debug_output:
                    print("Фаза нестабильна, сбрасываем счетчик")
            ...
            # Выводим информацию о времени без сигнала только через заданные интервалы
            if debug_output and show_terminal_info:
                minutes, seconds = divmod(int(silent_time), 60)
                hours, minutes = divmod(minutes, 60)
                ...
                if last_max_amplitude > 0:
                    status_str = f"\rБез сигнала: {time_str} | Последний: A={last_max_amplitude:.0f}, "
                    status_str += f"Ф={last_max_phase:.0f}, ΔФ={last_phase_diff:.0f} | {last_beep_timestamp}"
                else:
                    status_str = f"\rБез сигнала: {time_str} | Порог: {adaptive_threshold:.0f} | Режим: "
                    status_str += f"{'Адаптивный' if threshold_update_enabled else 'Фиксированный'}"
                
                # Перезаписываем строку
                print(status_str + " " * 40, end="\r", flush=True)

            # Показываем предупреждения в определенные моменты времени
            for period in silent_warning_periods:
                if silent_time >= period and not silent_warnings_shown[period]:
                    silent_warnings_shown[period] = True
                    period_min, period_sec = divmod(period, 60)
                    # Добавляем предупреждение в лог вместо прямого вывода
                    warning_message = f"⚠️ ВНИМАНИЕ! Прошло {period_min}м {period_sec}с без обнаружения сигнала!"
                    add_log_message(warning_message)
...
# Добавить в блок импортов в начале файла
import sys
import io
from contextlib import redirect_stdout

# Добавить после других глобальных переменных
# Переменные для контроля вывода в консоль
original_stdout = sys.stdout
log_messages = []  # Сюда будем собирать важные сообщения для отображения позже
info_message = ""  # Здесь будет храниться строка с текущей информацией о состоянии
last_terminal_update = time.time()
terminal_update_interval = 1.0  # Обновление информации раз в секунду
max_log_messages = 5  # Максимальное количество сообщений для отображения
suppress_graph_built_messages = True  # Подавлять ли сообщения о построении графика

# Функция для перехвата вывода update_graph
def update_graph_wrapper(*args, **kwargs):
    global info_message, suppress_graph_built_messages
    
    # Если нужно подавить сообщения о построении графика
    if suppress_graph_built_messages:
        # Перехватываем вывод
        f = io.StringIO()
        with redirect_stdout(f):
            result = update_graph(*args, **kwargs)
        
        # Анализируем перехваченный вывод
        output = f.getvalue()
        if not output.strip().startswith("График для кнопки"):
            # Если это не сообщение о построении графика, выводим его
            print(output, end="")
    else:
        # Если подавление отключено, просто вызываем функцию
        result = update_graph(*args, **kwargs)
    
    return result

# Функция для добавления важных сообщений в лог
def add_log_message(message):
    global log_messages, max_log_messages
    log_messages.append(message)
    # Ограничиваем количество сообщений
    if len(log_messages) > max_log_messages:
        log_messages.pop(0)  # Удаляем самое старое сообщение

# Функция для отображения текущего состояния
def display_status():
    global info_message, log_messages
    
    # Очищаем терминал
    print("\033c", end="")
    
    # Печатаем информационную строку
    print(info_message)
    
    # Печатаем разделитель
    print("-" * 80)
    
    # Печатаем последние сообщения
    if log_messages:
