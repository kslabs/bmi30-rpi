...
def switch_graph(label, buttons, light_green, text_box, current_oscilloscope, update_needed, show_plot, fig):
    ...
    if label == '0':
        ...
        text_box.set_text('Выключаем осциллограмму, но работаем')
        ...
        text_box.set_text('Зоны ожидания метки верхней антенны')
        print(f"Переключение на график {current_oscilloscope[0]}")
    elif label == '5':
        current_oscilloscope[0] = int(label)
        show_plot[0] = True
        ...
        text_box.set_text('Зоны ожидания метки нижней антенны')
        print(f"Переключение на график {current_oscilloscope[0]}")
    elif label == '6':
        current_oscilloscope[0] = int(label)
        show_plot[0] = True
...
def update_graph(show_plot, update_needed, current_oscilloscope, start_sample, num_samples, dataADC_P1, dataADC_L1, line1, line2, ax, fig):
    if show_plot:
        len_P1 = len(dataADC_P1)
        len_L1 = len(dataADC_L1)
        
        if current_oscilloscope == 1:
            # Используем одинаковую длину для обоих графиков
            data_length = min(len_P1, len_L1)
            x_axis = np.arange(data_length)
            line1.set_data(x_axis, dataADC_P1[:data_length])
            line2.set_data(x_axis, dataADC_L1[:data_length])
        elif current_oscilloscope == 2:
            dataADC_P1[550]= dataADC_P1[550] + 10000
            dataADC_P1[350]= dataADC_P1[350] + 10000
            dataADC_P1[1320]= dataADC_P1[1320] + 10000
            dataADC_P1[1520]= dataADC_P1[1520] + 10000
            
            x_axis = np.arange(len_P1)
            line1.set_data(x_axis, dataADC_P1)
            line2.set_data(x_axis, np.zeros(len_P1))
        elif current_oscilloscope == 3:
            ...
            x_axis = np.arange(len_L1)
            line1.set_data(x_axis, np.zeros(len_L1))
            line2.set_data(x_axis, dataADC_L1)
        elif current_oscilloscope == 4:
            # Зоны ожидания для верхней антенны
            x_axis1 = np.arange(350, 551)
            x_axis2 = np.arange(1320, 1521)
            line1.set_data(x_axis1, dataADC_P1[350:551])  # Фиолетовый график
            line2.set_data(x_axis2, dataADC_P1[1320:1521])  # Шоколадный график
            ax.set_xlim(300, 1600)  # Установим границы для лучшей видимости обеих зон
        elif current_oscilloscope == 5:
            # Зоны ожидания для нижней антенны
            x_axis1 = np.arange(350, 551)
            x_axis2 = np.arange(1320, 1521)
            line1.set_data(x_axis1, dataADC_L1[350:551])  # Фиолетовый график
            line2.set_data(x_axis2, dataADC_L1[1320:1521])  # Шоколадный график
            ax.set_xlim(300, 1600)  # Установим границы для лучшей видимости обеих зон
