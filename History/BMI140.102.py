...
dataINP1 = []       # массив компенсации постоянной составляющей
...
try:                        # для аккуратного принудительного останова программы с закрытием портов
    ...
    while True:
        ...
        if len(dataADC1) > CHUNK4:                               # количество данных больше чем одно считывание
            for i in range(0, CHUNK):                      # находим начало цикла 12 периодов первому встреченному переходу меньше/больше нуля
                if start_adr < 0 and dataADC1[i] < 8 and dataADC1[i+1] > 8:     # находим стартовый адрес, если его нет (-1)
                    start_adr = i
                    break       
