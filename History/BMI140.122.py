...
def callback(in_data, frame_count, time_info, status):
    ...
    if start_adr < 10:
        ...
        #slices_p = dataADC1[start_adress + (1920 * y_indices[:, None]): stop_adress + (1920 * y_indices[:, None])]
        #slices_n = dataADC1[start_adress + 955 + (1920 * y_indices[:, None]): stop_adress + 955 + (1920 * y_indices[:, None])]

        #data_to_sum_p = np.mean(slices_p, axis=0)
        #data_to_sum_n = np.mean(slices_n, axis=0)
