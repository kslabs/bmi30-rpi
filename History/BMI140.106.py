...
CHUNK = 15360        # Устанавливаем количество семлов считывания  (46080 расчетное)  32765  (23041 подобрано опытным путем); 3839: 2 периода 200Гц; 
...
def callback(in_data, frame_count, time_info, status):
    ...
    #dataADC1.extend(list(struct.unpack(str(CHUNK) + 'h', dataADC)))

    # print(len(dataADC1))
    #count_inp_ADC += 1
    
    #print(count_inp_ADC, len(dataADC1), time.time())
    ...
    #print(len(dataADC1))
    #print(count_inp_ADC, len(dataADC1), status, time.time_ns() - timerP)
...
#    p.terminate()
    ...
    while stream.is_active():
        ...
        if led_line.get_value() != fled_line and len(dataADC1) > CHUNK:                               # количество данных больше чем одно считывание
        #if 1==1:
            led_line = led_line.get_value()
            fcount_inp_ADC = count_inp_ADC
            #led_line.set_value(0)
            
            #led_line.set_value(1)
                 
            for i in range(0, CHUNK0):                      # находим начало цикла 12 периодов первому встреченному переходу меньше/больше нуля
                ...
                if  dataADC1[i] < 10 and dataADC1[i+1] > 10: 
                    start_adr = i
                ...
                if 1==0:
                    if dataADC1[i] > 0: 
                        ...
                        if dataADC1[i] > dataDC[i]:
                            ...
                            dataDC[i] = dataDC[i] - tau
                        dataINP1[i] = (dataADC1[i] - dataDC[i]) * 4
                    else:
            ...
            dataADC1[start_adr+31] = dataADC1[start_adr+31] + 5000
            dataADC1[start_adr+44] = dataADC1[start_adr+44] + 5000
            dataADC1[start_adr+70] = dataADC1[start_adr+70] + 5000
            ...
            if waveform_output_display == 2:                         # отображаем осциллограмму входного сигнала с компенсированным DC
                display_line.set_ydata(dataINP1[start_adr:(CHUNK + start_adr):])
                fig.canvas.draw()
                fig.canvas.flush_events()
            if waveform_output_display == 3:                         # отображаем осциллограмму DC
                display_line.set_ydata(dataDC[start_adr:(CHUNK + start_adr):])
                fig.canvas.draw()
                fig.canvas.flush_events()
