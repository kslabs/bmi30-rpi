...
history = np.zeros((HISTORY_SIZE, CHUNK), dtype=np.float32)    # Массив с историями 
...
def callback(in_data, frame_count, time_info, status):
    ...
    new_data = np.frombuffer(in_data, dtype=np.int16)   # переводим входные данные в массив NumPy
    history[1:] = history[:-1]                          # сдвигаем историю повех существующих элементов массива
    history[0] = new_data[:CHUNK]                       # обновляем новыми входными данными
    ...
    try:
        if len(history[0]) >= CHUNK:    
            sync_mask = (history[0][:-24] > 6000) # & (history[0][1:-23] > 7000) # & (history[0][24:] > 6000)
            sync_indices = np.where(sync_mask)[0]           # находим номер первого семпла, соответствующего условиям

            if len(sync_indices) > 0 and len(history[0]) > sync_indices[0]:
                start_adr = sync_indices[0]                 # первый индекс синхронизации
                history[0] = history[0][start_adr:]         # обрезаем массив начиная со стартового адреса. удаляем ненужные данные до точки синхронизации
        else:
            sync_mask = (False)
            sync_indices = 0
    except Exception as e:
    ...
    print(CHUNK, len(history[0]), np.max(history[0]), sync_indices)
