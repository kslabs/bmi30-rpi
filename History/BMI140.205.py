...
def process_data(adc_P0, adc_L0, valid_starts, num_events):
    # ??????????????? ????????? ???????? ??? ???????? ????????
    front_values_P = np.empty((num_events, chunk_out2), dtype=np.int64)
    spad_values_P = np.empty((num_events, chunk_out2), dtype=np.int64)
    front_values_L = np.empty((num_events, chunk_out2), dtype=np.int64)
    spad_values_L = np.empty((num_events, chunk_out2), dtype=np.int64)
