...
BUTTON_CONFIG = {
    ...
    3: {
        ...
        'add_markers': [330, 530, 1370, 1570]  # Метки зон 3255, 3495
    },
    4: {
        'text': 'Зоны ожидания',
        'show_plot': True,
...
distortion_index = 0  # Значение индекса искажения по умолчанию
gradient_threshold = 0  # Значение порога по умолчанию

correlation_changes = []  # История изменений формы сигнала
correlation_change_threshold = None  # Порог изменения формы сигнала
...
def update_graph(
    show_plot, update_needed, current_oscilloscope, start_sample, num_samples,
    ...
    antiphase_correlation_sum, adaptive_correlation_threshold,
    correlation_detection_count, correlation_detection_threshold,
    ...
    if show_plot:
        ...
        if current_oscilloscope == 1:
            ...
            line1.set_data(np.arange(len(dataADC_P1)), dataADC_P1)
            line2.set_data(np.arange(len(dataADC_L1)), dataADC_L1)
            ax.set_xlim(0, len(dataADC_P1))
            ...
            # Рассчитываем разницу корреляций
            correlation_diff = cross_correlation_sum - antiphase_correlation_sum
            
            # Убираем DC-компоненту из разницы корреляций
            dc_component = calculate_correlation_dc(correlation_diff)
            correlation_diff_ac = correlation_diff - dc_component
            
            # Очищаем график для перерисовки
            ax.clear()
            
            # Обновляем данные для отображения
            shifts = np.arange(-CROSS_CORRELATION_SHIFTS, CROSS_CORRELATION_SHIFTS + 1)
            ax.plot(shifts, correlation_diff_ac, 'b-', label='AC-компонента', linewidth=1.5)
            ax.plot(shifts, dc_component, 'r-', label='DC-компонента', linewidth=1.5)
            
            # Также обновляем основные линии для совместимости
            line1.set_data(shifts, correlation_diff_ac)
            line2.set_data(shifts, dc_component)
            
            # Настраиваем оси и заголовок
            ax.set_xlim(-CROSS_CORRELATION_SHIFTS, CROSS_CORRELATION_SHIFTS)
            
            # Смещаем ось X вниз на 20% от диапазона Y
            max_ac = np.max(np.abs(correlation_diff_ac)) * 1.2 or 1.0
            max_dc = np.max(np.abs(dc_component)) * 1.2 or 1.0
            y_max = max(max_ac, max_dc, 100)  # Минимальная высота 100
            y_range = 2 * y_max  # Полный диапазон оси Y
            
            # Устанавливаем новые границы Y с учетом смещения оси X вниз
            y_min = -y_max * 0.9  # Смещаем нижнюю границу вниз на 20%
            y_max = y_max * 1.5   # Оставляем верхнюю границу как есть
            ax.set_ylim(y_min, y_max)
            
            # Перемещаем ось X вниз на 20% от диапазона Y
            ax.spines['bottom'].set_position(('axes', -0.2))
            
            # Улучшаем внешний вид
            ax.set_title("График 29: AC и DC компоненты")
            ax.grid(True)
            ax.legend(loc="upper right")
            
            # Принудительно обновляем график
            fig.canvas.draw_idle()
            update_needed = False  # Отключаем дополнительное обновление
...
_spectrum_cache = {}  # Глобальный словарь для кэширования

def calculate_spectrum(data, fs, f_min, f_max):
    """Расчет спектра сигнала с кэшированием"""
    global _spectrum_cache
    
    # Быстрый выход на случай пустых данных
    if len(data) == 0:
        return np.array([0]), np.array([0])
    
    # Хэширование входных данных для проверки кэша
    data_hash = hash(data.tobytes())
    cache_key = (data_hash, fs, f_min, f_max)
    
    # Проверка кэша
    if cache_key in _spectrum_cache:
        return _spectrum_cache[cache_key]
    
    # Обычное вычисление, если кэш-промах
    n_fft = 32768
    data_centered = data - np.mean(data)
    window = np.blackman(len(data_centered))
    windowed_data = data_centered * window
    
    # Оптимизация: использование меньшего n_fft для коротких сигналов
    if len(windowed_data) < 4096:
        n_fft = 8192
    
    # Вычисление спектра
    spectrum = np.fft.fft(windowed_data, n=n_fft)
    spectrum = np.abs(spectrum[:n_fft // 2])  # Берем только положительные частоты
    
    # Вычисление частотной оси
    freqs = np.fft.fftfreq(n_fft, d=1/fs)[:n_fft // 2]
    
    # Ограничение по диапазону частот
    valid_indices = (freqs >= f_min) & (freqs <= f_max)
    freqs = freqs[valid_indices]
    spectrum = spectrum[valid_indices]
    
    # Сохраняем в кэш
    _spectrum_cache[cache_key] = (freqs, spectrum)
    
    # Ограничиваем размер кэша
    if len(_spectrum_cache) > 20:
