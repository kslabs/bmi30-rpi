...
mark11 =  335   # начало рабочей границы фронта -DC
mark12 =  575   # конец рабочей границы фронта -DC
mark13 = 1295   # начало рабочей границы спада -DC
mark14 = 1535   # конец рабочей границы спада -DC
...
#    p.terminate()
    ...
    while stream.is_active():
        ...
        if count_sample != count_start and len(dataADC1) > CHUNK:                               # количество данных больше чем одно считывание
            ...
            for y in range(start_adr + mark11, CHUNK, 1920):
                for i in range(start_adr + y + mark11, start_adr + y + mark12):   # вычисляем постоянную составляющую DC c АРУ на 10% от максимума переменной составляющей
                    if dataADC1[i] > 0: 
                   
                        if dataADC1[i+y] > dataDC[i]:
                            dataDC[i] = dataDC[i] + tau
                        else:
                            dataDC[i] = dataDC[i] - tau
                        
                            dataINP1[i] = (dataADC1[i+y] - dataDC[i]) *10
                    else:
            ...
            if waveform_output_display == 1:                         # отображаем осциллограмму сырого входного сигнала
                ...
                display_line.set_ydata(dataADC1[CHUNK_start_out:CHUNK_out+CHUNK_start_out:])
                fig.canvas.draw()
