...
version = 52.11  # Обновляем версию
...
try:
    ...
    while True:
        ...
        # Обнаружение сигнала на основе разницы корреляций (график 29)
        if correlation_based_detection and len(cross_correlation_sum) > 0 and len(antiphase_correlation_sum) > 0:
            # Вычисляем разницу корреляций
            correlation_diff = cross_correlation_sum - antiphase_correlation_sum
            
            # Получаем значение в точке +45 (индекс 145 в массиве)
            if len(correlation_diff) > correlation_index:
                correlation_value = correlation_diff[correlation_index]
                ...
                if correlation_value > correlation_threshold:
                    ...
                    if correlation_detection_count >= correlation_detection_threshold:
                        ...
                        print(f"Сигнал обнаружен по разнице корреляций! Значение: {correlation_value:.1f}, Порог: {correlation_threshold}, Счетчик: {correlation_detection_count}")
                else:
                    # Сбрасываем счетчик, если значение ниже порога
                    correlation_detection_count = 0
                    
                    # Сигнал не обнаружен - выключаем звук
                    stop_beep()
                
                # Выводим отладочную информацию
                if debug_output:
                    print(f"Разница корреляций (+45): {correlation_value:.1f} / {correlation_threshold} | Счетчик: {correlation_detection_count}/{correlation_detection_threshold} | BEEP: {'ON' if correlation_detection_count >= correlation_detection_threshold else 'OFF'}")
