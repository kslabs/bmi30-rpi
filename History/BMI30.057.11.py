...
adaptive_correlation_threshold = 10000
...
def save_correlation_dc():
    ...
    if 'correlation_dc_component' in globals() and correlation_dc_component is not None:
        ...
        if debug_output:
            print(f"DC-компонента корреляции сохранена")
...
def load_correlation_dc():
    ...
    try:
        ...
        print("Файл с DC-компонентой корреляции не найден, используются нулевые значения")
        correlation_dc_component = None
...
try:
    ...
    while True:
        ...
        continue_loop, update_needed = update_graph(
            ...
            None,  # correlation_changes
            None,  # correlation_change_threshold
            correlation_sound_active  # Добавляем передачу параметра
        )
        ...
        # Обнаружение сигнала на основе изменений формы графика разности корреляций
        if correlation_based_detection and len(cross_correlation_sum) > 0 and len(antiphase_correlation_sum) > 0:
            # Вычисляем разницу корреляций
            correlation_diff = cross_correlation_sum - antiphase_correlation_sum
            
            # Вычисляем DC компоненту с медленной адаптацией
            correlation_dc = bmi30_plt.compute_correlation_dc(correlation_diff, alpha=0.005)
            
            # Удаляем DC из сигнала для получения AC-компоненты
            correlation_ac = correlation_diff - correlation_dc
            
            # Вычисляем характеристики шума и сигнала для отладки
            noise_area = list(range(0, 50)) + list(range(150, 200))
            signal_area = list(range(50, 150))
            
            # Используем AC-компоненту для анализа шума
            noise_values = [correlation_ac[i] for i in noise_area]
            noise_mean = np.mean(noise_values)
            noise_std = np.std(noise_values) + 100
            
            # Используем AC-компоненту для анализа сигнала
            signal_values = [correlation_ac[i] for i in signal_area]
            signal_mean = np.mean(signal_values)
            signal_max = max([abs(val - noise_mean) for val in signal_values])
            
            # Получаем SNR и флаг обнаружения для всего графика с пониженным порогом
            # Передаем AC-компоненту вместо полного сигнала
            snr, signal_detected = bmi30_plt.compute_adaptive_correlation_threshold(
                correlation_diff=correlation_ac, 
                sound_active=correlation_sound_active, 
                current_threshold=adaptive_correlation_threshold,
                base_threshold_ratio=0.1,  # 10% добавки к порогу
                adaptation_rate=0.05       # Скорость адаптации порога
            )
            
            # Сохраняем DC каждые 180 секунд
            current_time = time.time()
            if current_time - last_dc_save_time >= DC_SAVE_INTERVAL:
                save_correlation_dc()
            
            # Проверяем обнаружение сигнала
            if signal_detected:
                ...
                if correlation_detection_count >= correlation_detection_threshold:
                    ...
                    print(f"Сигнал обнаружен по изменению формы графика! "
                          f"SNR: {snr:.2f}/{adaptive_correlation_threshold*1.1:.2f}, "
                          f"Счетчик: {correlation_detection_count}/{correlation_detection_threshold}")
            else:
                # Сбрасываем счетчик, если сигнал не обнаружен
                correlation_detection_count = 0
                
                # Сигнал не обнаружен - выключаем звук и обновляем флаг
            ...
            if debug_output:
                print(f"Форма графика: SNR={snr:.2f}/{adaptive_correlation_threshold*1.1:.2f} | "
                      f"Сигнал макс: {signal_max:.0f}, среднее: {signal_mean:.0f} | "
                      f"Шум: ср={noise_mean:.0f}, std={noise_std:.0f} | "
                      f"Счетчик: {correlation_detection_count}/{correlation_detection_threshold} | "
