...
time_wr = start_time       # Для отсчета ВРЕМЕНИ ЗАПИСИ DC
...
tau = 0                     # постоянная времени вычисления DC
...
def callback(in_data, frame_count, time_info, status):
    ...
    # синхронизация
    if len(dataADC1) > CHUNK3:                               # количество данных больше чем один пакет
    ...
    # изменяем состояние вывода для обозначения конца обновления данных
...
def data_processing():                  # Обработка данных АЦП
    max_variable = 0
    global count_start
    start_adr = 0
    global mark11,mark12,mark13,mark14,mark15
    global CHUNK
    global CHUNK2
    global dataDC
    ...
    for i in range(start_adr, CHUNK + start_adr):   # вычисляем постоянную составляющую DC c АРУ на 10% от максимума переменной составляющей
        if 1==0:
            if dataADC1[i] > 0: 
                if 1==0 and dataADC1[i] > max_variable:          # вычисляем максимум по +AC
                ...
                if 1==0 and (dataADC1[i] * -1) > max_variable:   # вычисляем максимум по -AC        
...
if waveform_output_display:
    ...
    mark15 =  965   # 4 метка -DC 386   # 1 метка -DC
    mark16 = 1354   # 3 метка -DC
    mark17 = 2310   # 2 метка -DC
    mark18 =  386   # 1 метка -DC
