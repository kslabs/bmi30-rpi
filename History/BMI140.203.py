...
adc_L0f = np.zeros(chunk_out2, 'int64')       # массив средних данных левого канала по фронту
adc_L0s = np.zeros(chunk_out2, 'int64')       # массив средних данных девого канала по спаду
...
def callback(in_data, frame_count, time_info, status):
    ...
    global adc_L0, adc_P0,adc_P0f, adc_P0s, adc_L0f, adc_L0s, accum_DC_front_P, accum_DC_spad_P, accum_DC_front_L, accum_DC_spad_L
    global count_accum, accumulation_count, chunk_out2, threshold, sinhro, start_index_front, start_index_spad
    ...
    condition_P = (adc_P0[:-35] < threshold) & (adc_P0[1:-34] > threshold) & \
                (adc_P0[28:-7] > threshold) & (adc_P0[35:] < threshold)
    condition_L = (adc_P0[:-35] < threshold) & (adc_P0[1:-34] > threshold) & \
                (adc_P0[28:-7] > threshold) & (adc_P0[35:] < threshold)
    
    # находим все индексы, где условия выполняются
    valid_starts_P = np.where(condition_P)[0]
    valid_starts_L = np.where(condition_L)[0]
    ...
    for start_index in valid_starts_P:
        front_start = start_index + start_index_front
        spad_start = start_index + start_index_spad
    
        # добавление значений во фронты       
        if front_start + chunk_out2 <= len(adc_P0):
            front_values_P.append(adc_P0[front_start:front_start + chunk_out2])

        # добавление значений в спады
        if spad_start + chunk_out2  <= len(adc_P0):
            spad_values_P.append(adc_P0[spad_start:spad_start + chunk_out2])
            
    for start_index in valid_starts_L:
        front_start = start_index + start_index_front
        spad_start = start_index + start_index_spad
    
        # добавление значений во фронты       
        if front_start + chunk_out2 <= len(adc_L0):
            front_values_L.append(adc_L0[front_start:front_start + chunk_out2])

        # добавление значений в спады
        if spad_start + chunk_out2 <= len(adc_L0):
            spad_values_L.append(adc_L0[spad_start:spad_start + chunk_out2])
            
    # преобразование списков в массивы для усреднения
    front_values_array_P = np.array(front_values_P)
    spad_values_array_P = np.array(spad_values_P)
    front_values_array_L = np.array(front_values_L)
    spad_values_array_L = np.array(spad_values_L)
        
    if front_values_array_P.size > 0:
        adc_P0f = np.round(np.mean(front_values_array_P, axis=0)).astype(int)
    if spad_values_array_P.size > 0:
        adc_P0s = np.round(np.mean(spad_values_array_P, axis=0)).astype(int)
    
    if front_values_array_L.size > 0:
        adc_L0f = np.round(np.mean(front_values_array_L, axis=0)).astype(int)
    if spad_values_array_P.size > 0:
        adc_L0s = np.round(np.mean(spad_values_array_L, axis=0)).astype(int)
