...
def callback(in_data, frame_count, time_info, status):
    ...
    if len(dataADC1) > CHUNK:
        ...
        for i in range(0, CHUNK):
            if flag_synchro or (dataADC1[i] < 6000 and dataADC1[i+1] > 6000 and dataADC1[i+28] > 6000 and dataADC1[i+35] < 6000):
                flag_synchro = 1
...
if waveform_output_display:
    ...
    if waveform_output_display == 14:
        ...
        line_p, = ax.plot(x_axis, data_to_sum_p, label='data_to_sum_p', color='blue', linestyle='-')  # ?????? ??? data_to_sum_p
        line_n, = ax.plot(x_axis, data_to_sum_n, label='data_to_sum_n', color='red', linestyle='-')   # ?????? ??? data_to_sum_n
    ...
    while running and stream.is_active():
        ...
        if start_adr > 9:
            ...
            if waveform_output_display == 14:                         # отображаем осциллограмму DC
                ...
                line_p.set_ydata(data_to_sum_p)  # График массива data_to_sum_p
                line_n.set_ydata(data_to_sum_n)  # График массива data_to_sum_n
