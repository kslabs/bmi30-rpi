...
version = 10.00  # Обновляем версию
...
def synchronize_data(data):
    ...
    for i in range(len(data) - 90):
        if data[i] > threshold1 and data[i + 25] > threshold1 and data[i + 82] > threshold2:
            data = data[i:]
            start_sinchronize = i
...
# Конфигурируем pwm
pwm3 = HardwarePWM(pwm_channel=3, hz=213.35120042321, chip=2)  # OnPWR 196.95120042321
pwm2 = HardwarePWM(pwm_channel=2, hz=200.00920042321, chip=2)
