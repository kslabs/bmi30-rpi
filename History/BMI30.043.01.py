...
version = 42.00  # Обновляем версию
...
stable_phase_threshold = 8      # Порог стабильности фазы (±5 отсчетов)
...
threshold_margin = 1.25         # Множитель для порога (20% запас)
...
# Добавьте после объявления других параметров детектирования (примерно строка 400-420)
...
def save_settings(filename, settings):
    ...
    # Сохраняем границы диапазона поиска фазы
    settings['phase_shift_min'] = phase_shift_min
    settings['phase_shift_max'] = phase_shift_max
...
def load_settings(filename):
    ...
    global phase_shift_min, phase_shift_max
    ...
    try:
        ...
        # Загружаем границы диапазона поиска фазы
        phase_shift_min = settings.get('phase_shift_min', 0)
        phase_shift_max = settings.get('phase_shift_max', 75)
...
try:
    ...
    # Экспортируем границы диапазона поиска фазы в bmi30_plt.py
    import bmi30_plt
    ...
    while True:
        ...
        if stabilization_cycles < STABILIZATION_LIMIT:
            ...
            if max_amp_15 > adaptive_threshold:
                ...
                if threshold_update_enabled:
                    ...
                    delta_pfase_signal_on = abs(max_phase_15 - prev_max_phase_15)
                    if debug_output:
        ...
        if debug_output:
            signal_threshold = max_amp_15 * threshold_margin  # Вычисляем пороговое значение на основе текущего сигнала
            ...
            if threshold_diff > 0:  # Порог увеличивается
                arrow = "↑"  # Стрелка вверх для увеличения
            else:  # Порог уменьшается или не меняется
                arrow = "↓"  # Стрелка вниз для уменьшения
            ...
            f"Порог: {arrow} {change_value:3.0f}"  # Используем вычисленные arrow и change_value
            )
            print(info_str)
