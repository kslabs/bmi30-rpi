...
waveform_output_display = 1     # Разрешение вывода на экран: 0-без вывода; 1-сырого сигнала; 2-сигнала без постоянной; 3-постоянная составляющая сигнала
                                # 4-сумма по фронту; 5-сумма по спаду; 6-сумма 9 циклов без DC; 7- тест; 8-вторая производная; 9-тест
...
CHUNK = 23040        # Устанавливаем количество семлов считывания  (46080 расчетное)  32765  (23040 подобрано опытным путем); 3839: 2 периода 200Гц; 
...
def callback(in_data, frame_count, time_info, status):
    ...
    global dataADC, dataADC1, dataADC2, dataADC3, dataADC4, dataADC5, dataADC6, dataADC7,dataADC8, dataADC9
    global start_adr, count_start, data_to_sum_p, data_to_sum_n, start_adress, stop_adress, tau_start_adr, flag_synchro
    flag_synchro = 0
    ADCmax = 0
    ...
    if len(dataADC1) > CHUNK:
        ...
        for i in range(0, CHUNK):
            #if flag_synchro or (dataADC1[i] < 6500 and dataADC1[i+1] > 6500 and dataADC1[i+24] > 6500):
            #if flag_synchro or (dataADC1[i] < 12000 and dataADC1[i+1] > 12000 and dataADC1[i+27] > 12000 and dataADC1[i+33] < 12000):
            if dataADC1[i] < -13000 and dataADC1[i+11] > -13000 and dataADC1[i+24] < -13000 and dataADC1[i+40] > 13000:
                flag_synchro = 1

                ADCmax = max(ADCmax, dataADC1[i])       # находим максимум в массиве

                # Удаляем начало массива
                start_adr = i
                dataADC5, dataADC4, dataADC3 = np.roll([dataADC4, dataADC3, dataADC2], shift=1) 
                #shift_amount = CHUNK + (int(i / 1.1) if i > 6 else 1 if i > 4 else 0 if i > 2 else -1)
                shift_amount = CHUNK + (i+1 if i > 2 else 0 if i < 2 else i-1 )
                dataADC2 = np.copy(dataADC1[:CHUNK])
                dataADC1 = dataADC1[shift_amount:]
                break
            else:
                flag_synchro = 0
    ...
    #print (dataADC1[0], start_adr, flag_synchro)
    # Загрузка новых данных АЦП
    ...
    #dataADC1, dataADC0 = new_data[:, 0], new_data[:, 1]
    #dataADC1[:len(new_data)] = new_data[:, 0]
    dataADC1 = np.concatenate((dataADC1, new_data[:, 0]))
    #dataADC1[:len(new_data)] = new_data[:, 0]
    
    
    
    #print(len(dataADC1), len(new_data))
    
    # Выделение рабочей зоны и суммирование по фронту и спаду
    if start_adr < 10 and tau_start_adr == 0:
        count_start += 1
        
        dataADC = np.concatenate((dataADC9, dataADC8, dataADC7, dataADC6, dataADC5,dataADC4, dataADC3, dataADC2, dataADC1))

        y_indices_p = np.arange(int(len(dataADC) / 1920)) * 1920
        y_indices_n = np.arange(int(len(dataADC) / 1920)) * 1920 + 910
...
# Выравнивание осциллограммы по горизонтальной оси относительно центра
def align_to_center(data):
    x = np.arange(len(data))  # индексы для данных
    coefficients = np.polyfit(x, data, 1)  # линейная регрессия (1 степень)
    trend = np.polyval(coefficients, x)  # вычисляем тренд
    aligned_data = data - trend  # выравниваем, вычитая тренд
    return aligned_data
...
try:
    ...
    dataDC_p = np.load('tab_DC_p.npy')
    dataDC_n = np.load('tab_DC_n.npy')
...
pwm1 = HardwarePWM(pwm_channel=3, hz=200.009200423219496656201954465359449386596, chip=2)    # pwm_channel 1: 3-GPIO_18; 2-GPIO_19; 
...
if waveform_output_display:
    ...
    if waveform_output_display == 5:
        x_axis = np.arange(0, CHUNK_out2 * 2, 2)
        display_line, = ax.plot(x_axis, np.random.rand(CHUNK_out2), '.-')
        plt.grid(which='major', alpha=0.6, color='green', linestyle='--', linewidth=1.4)
        ax.set_ylim(-8000, 100)
        ax.ser_xlim = (0, CHUNK_out2)
        #ax = np.linspace(100.0, 500.0)
    ...
    if waveform_output_display == 10:
        ...
        ax.ser_xlim = (0, CHUNK_out2)
        #ax = np.linspace(100.0, 500.0)
    if waveform_output_display == 14:
        x_axis = np.arange(0, CHUNK_out2)
        line_p, = ax.plot(x_axis, data_to_sum_p1, label='data_to_sum_p1', color='blue', linestyle='-')  # ?????? ??? data_to_sum_p
        line_n, = ax.plot(x_axis, data_to_sum_n1, label='data_to_sum_n1', color='red', linestyle='-')   # ?????? ??? data_to_sum_n
...
tau3 = 100
tau4 = 1000
trigger_treshold1 =  9000
trigger_treshold2 =  18000
trigger_treshold3 =  36000

if waveform_output_display == 6:
    plt.axhline(y=trigger_treshold1, color='b', linestyle='--')
    plt.axhline(y=trigger_treshold2, color='m', linestyle='--')
    plt.axhline(y=trigger_treshold3, color='r', linestyle='--')
    plt.axhline(y=-trigger_treshold1, color='b', linestyle='--')
    plt.axhline(y=-trigger_treshold2, color='m', linestyle='--')
    plt.axhline(y=-trigger_treshold3, color='r', linestyle='--')
...
count_delay_tag = 10 # счетчик длительности наличия метки        #  !!!!!!!!!!!!!!!!!!!  1
...
try:                        # для аккуратного принудительного останова программы с закрытием портов
    ...
    while running and stream.is_active():
        ...
        if start_adr > 9:
            ...
            data_to_sum_p2 = (data_to_sum_p * 200) -dataDC_p
            data_to_sum_n2 = (data_to_sum_n * 200) -dataDC_n
            data_to_sum_p1 = data_to_sum_p2 - np.mean(data_to_sum_p2)
            data_to_sum_n1 = data_to_sum_n2 - np.mean(data_to_sum_n2)
            
            dataINP11 = (data_to_sum_p1 - data_to_sum_n1) - dataDC
            
            dataINP11 = align_to_center(dataINP11)
            ...
            index_of_max = np.argmax(np.abs(dataINP_result))
            ...
            if flag_first_alignment and tau_start_adr==0 and dataINP_max > trigger_treshold2 + (int(count_delay_tag / 100) * 10000):
                ...
                if count_delay_notag > random_number:
                    update = np.floor(((np.power(abs(dataINP_result) + 1, 0.58) * np.log(abs(dataINP_result) + 1)) / 10) + 1).astype(np.int64)
                    dataDC += np.where(dataINP_result > 0, update, -update)
                    update_p = np.floor(((np.power(abs(data_to_sum_p1) + 1, 0.58) * np.log(abs(data_to_sum_p1) + 1)) / 10) + 1).astype(np.int64)
                    dataDC_p += np.where(data_to_sum_p1 > 0, update_p, -update_p)
                    update_n = np.floor(((np.power(abs(data_to_sum_n1) + 1, 0.58) * np.log(abs(data_to_sum_n1) + 1)) / 10) + 1).astype(np.int64)
                    dataDC_n += np.where(data_to_sum_n1 > 0, update_n, -update_n)
            ...
            # библиотечный дифференциал второй степени массива
            diff_all1 = np.diff(dataINP_res_tau, n=1)
            diff_all2 = np.diff(dataINP_res_tau, n=2)
            diff3 =  np.diff(dataINP_count, n=1)
            flag_diff3 = np.all(diff3[0:20] == -1)
            if dataINP_max < 20000:
                flag_diff4 = np.all(diff3[26:32] == 1)
            else:
                flag_diff4 = 0
            ...
            # Проверка наличия сбоя синхронизации
            if start_adr > 10:
            ...
            if count_time_detect > 10 and flag_first_alignment:      #!!!!!!!!!!!!!!!!!!!!!!! 10
                diff_sum_first = int(np.sum(abs(diff_all2[0:7])))
                diff_sum_second = int(np.sum(abs(diff_all2[7:14])))
                if count_delay_tag == 0:
                    max_mean = 0
                    #print(' ')
                if max_mean < abs(mean_dataINP_count):
                    max_mean = abs(mean_dataINP_count)
                #print(diff_sum_first)
                if flag_det_tag > 0 and abs(mean_dataINP_count) < 15 and diff_sum_first > 150 and diff3[0]>0 and max_mean <2 and flag_diff4 ==0:   # первые 10 сек не детектировать и проверять условие: средняя составляющая должна быть меньше 33. 
                    # Метка найдена

                        
                    print(marked_tau, f'{count_delay_tag:4}', f'Метка определена с макс. уровнем сигнала:{dataINP_max:6}', f'| Mean:{mean_dataINP_count:4}', f'/{max_mean:4}',f'/{diff_sum_first:4}','/', diff3[18:36], '      !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!       ', end='\r')
                    #dataDC = np.where(abs(dataINP11)<2000, np.where(dataINP11>0, dataDC+tau1,dataDC-tau1), np.where(abs(dataINP11)>000, np.where(dataINP11>0, dataDC+tau1, dataDC-tau1), np.where(dataINP11>0, dataDC+tau1,dataDC-tau1)))
                    count_delay_tag += 10
                    count_delay_notag = 0
                    ...
                    if count_delay_notag > random_number:
                        ...
                        #update = np.floor(((np.power(abs(dataINP_result) + 1, 0.58) * np.log(abs(dataINP_result)+1))/10)+1)
                    ...
                    if count_delay_tag >0:
                        ...
                        if abs(mean_dataINP_count) > max_mean:
                            max_mean = abs(mean_dataINP_count)
                    print(marked_tau, f'{count_delay_notag:4}', f'Метки нет.         Макс. уровень сигнала:{dataINP_max:6}', f'| Mean:{mean_dataINP_count:4}', f'/{max_mean:4}',f'/{diff_sum_first:4}','/', diff3[18:36], '      --------------------------------------      ', end='\r')
                    count_delay_tag = 0
                    count_delay_notag += 1
                    if count_delay_notag > 5:
                        led_line_tag.set_value(1)

            else:
                update = np.floor(((np.power(abs(dataINP_result) + 1, 0.58) * np.log(abs(dataINP_result) + 1)) / 10) + 1).astype(np.int64)
                #print('dataDC:', dataDC.dtype, dataINP_result.dtype, update.dtype)
                dataDC += np.where(dataINP_result > 0, update, -update)
                update_p = np.floor(((np.power(abs(data_to_sum_p1) + 1, 0.58) * np.log(abs(data_to_sum_p1) + 1)) / 10) + 1).astype(np.int64)
                dataDC_p += np.where(data_to_sum_p1 > 0, update_p, -update_p)
                update_n = np.floor(((np.power(abs(data_to_sum_n1) + 1, 0.58) * np.log(abs(data_to_sum_n1) + 1)) / 10) + 1).astype(np.int64)
            ...
            if waveform_output_display == 1:                         # отображаем осциллограмму сырого входного сигнала
                ...
                dataADC1[start_adr+0] = dataADC1[start_adr+0] + 5000
                dataADC1[start_adr+11] = dataADC1[start_adr+11] + 5000
                dataADC1[start_adr+24] = dataADC1[start_adr+24] + 5000
                dataADC1[start_adr+440] = dataADC1[start_adr+440] - 5000
                dataADC1[start_adr+680] = dataADC1[start_adr+680] - 5000
                dataADC1[start_adr+1300] = dataADC1[start_adr+1300] + 5000
                dataADC1[start_adr+1560] = dataADC1[start_adr+1560] + 5000
                #dataADC1[start_adr+1250] = dataADC1[start_adr+1250] - 5000
            ...
            if waveform_output_display == 6:                         # отображаем результирующую осциллограмму усредненного сигнала 7-х циклов с компенсированным DC
                ...
                dataINP_result1[index_of_max] = dataINP_result[index_of_max] + 6000
            ...
            if waveform_output_display == 9:                         # отображаем осциллограмму DC
                display_line.set_ydata(dataINP_count)
                fig.canvas.draw()
                fig.canvas.flush_events()

            if waveform_output_display == 14:                         # отображаем осциллограмму DC

                line_p.set_ydata(data_to_sum_p1)  # ????????? ????? ??? data_to_sum_p
                line_n.set_ydata(data_to_sum_n1)  # ????????? ????? ??? data_to_sum_n
    ...
    stream.close()
    p.terminate()
    print("\n\n2. MAIN: UART порт закрыт и светодиод/pwm выключен")
