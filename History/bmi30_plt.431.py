        ...
        if current_oscilloscope == 1:
            ...
            try:
                ...
                if len(valid_indices) > 0:
                    restricted_max_idx = valid_indices[np.argmax(amplitudes[valid_indices])]
                    max_idx = restricted_max_idx  # Индекс максимума в указанном диапазоне
                else:
                    max_idx = np.argmax(amplitudes)  # Если диапазон пустой, используем весь массив
                ...
                # Добавляем вертикальные линии, обозначающие границы диапазона
                ax.axvline(x=PHASE_SHIFT_MIN, color='green', linestyle='--', alpha=0.5, 
                           label=f'Мин. фаза ({PHASE_SHIFT_MIN})')
                ax.axvline(x=PHASE_SHIFT_MAX, color='green', linestyle='--', alpha=0.5, 
                           label='_nolegend_')  # Не отображать в легенде
                
                # Выделяем область поиска максимума
                ax.axvspan(PHASE_SHIFT_MIN, PHASE_SHIFT_MAX, alpha=0.1, color='green', 
                ...
                ax.grid(True)
                # Легенда отключена для предотвращения дублирования
                # create_unique_legend(ax)
                
                # Добавляем текстовую информацию вместо легенды
                info_text = f"Максимум: {max_amp:.0f} при сдвиге {max_shift}, диапазон поиска: {PHASE_SHIFT_MIN}-{PHASE_SHIFT_MAX}"
            ...
            try:
                ...
                # Выделяем область поиска максимума
                ax.axvspan(PHASE_SHIFT_MIN, PHASE_SHIFT_MAX, alpha=0.1, color='green', 
                           label='_nolegend_')  # Не отображаем в легенде
...
# Создайте вспомогательную функцию для обработки легенды
def create_unique_legend(ax):
    """Создает легенду только с уникальными элементами, без дублирования."""
    handles, labels = ax.get_legend_handles_labels()
    by_label = {}
    for handle, label in zip(handles, labels):
        if label not in by_label and label != '_nolegend_':
            by_label[label] = handle
    if by_label:  # Проверяем, что словарь не пустой
