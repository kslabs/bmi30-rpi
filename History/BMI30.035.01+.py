...
amplitude_threshold = 70000  # Порог амплитуды для сигнализации
...
def toggle_beep_mode(event=None):
    ...
    beep_counter = 0  # Сбрасываем счетчик при смене режима
...
try:
    ...
    while True:
        ...
        if max_amp_15 > amplitude_threshold:
            ...
            if abs(max_phase_15 - prev_max_phase_15) <= stable_phase_threshold:
                ...
                if consecutive_detections >= 1:
                    ...
                    if beep_mode == "continuous":
                        ...
                        # Прерывистый сигнал (3 включен, 1 выключен)
                        beep_counter = (beep_counter + 1) % 4
                        if beep_counter < 3:  # 0, 1, 2 -> включен
                            gpio13.on()
                        else:  # 3 -> выключен
                            gpio13.off()
                    
                    print(f"Сигнал обнаружен! Амплитуда: {max_amp_15:.0f}, Фаза: {max_phase_15}")
            ...
            # Выводим количество времени без сигнала
            ...
            if debug_output:
                if hours > 0:
                    print(f"Время без сигнала: {hours}ч {minutes}м {seconds}с")
                else:
                    print(f"Время без сигнала: {minutes}м {seconds}с")
                
                if max_amp_15 > 0:
                    print(f"Амплитуда ниже порога: {max_amp_15:.0f} < {amplitude_threshold}")
