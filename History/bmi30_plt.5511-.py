...
def initialize_buttons(fig, button_labels, button_width, callback_function):
    ...
    spacing = (1.0 - 0.1 - 0.1) / 16  # Равномерное распределение между кнопками
    ...
    for i, label in enumerate(button_labels):
        if i < 16:
            ...
            # Второй ряд кнопок (16-31) - исправляем формулу позиционирования
            idx = i - 16  # Вычисляем индекс второго ряда кнопок
            ax_button = plt.axes([0.1 + (idx % 16) * spacing, 0.85, button_width, 0.05])
...
def update_graph(show_plot, update_needed, current_oscilloscope, start_sample, num_samples, 
                ...
                reference_correlation_diff=None, pattern_difference_threshold=None): # Добавляем новые параметры
    ...
    if show_plot:
        ...
        if current_oscilloscope == 1:
            ...
            try:
                ...
                ax.text(0.05, 0.70, f'Амплитуда: {max_amp - min_amp:.0f}',
            ...
            # Очищаем график
            ax.clear()
            ax.set_title('Разница корреляций (стандартная - противофазная)')
            ax.grid(True)
            ...
            if cross_correlation_sum is not None and antiphase_correlation_sum is not None and len(cross_correlation_sum) > 0 and len(antiphase_correlation_sum) > 0:
                ...
                # Определяем max_abs_val перед его использованием
                max_abs_val = np.max(np.abs(correlation_diff))
                
                # Строим график разницы
                diff_line, = ax.plot(shifts, correlation_diff, 'green', linewidth=1.5, label='Разница корреляций')
                
                # Находим максимальное значение разницы и его смещение
                max_idx = np.argmax(correlation_diff)
                max_shift = max_idx - 100  # Центрируем относительно 0
                max_value = correlation_diff[max_idx]
                
                # Находим минимальное значение разницы и его смещение
                min_idx = np.argmin(correlation_diff)
                min_shift = min_idx - 100
                min_value = correlation_diff[min_idx]
                
                # Добавляем вертикальные линии на максимуме и минимуме
                ax.axvline(x=max_shift, color='orange', linestyle='--', linewidth=1)
                ax.axvline(x=min_shift, color='blue', linestyle='--', linewidth=1)
                
                # Добавляем вертикальную линию в точке +45 (где измеряется значение для обнаружения)
                ax.axvline(x=45, color='red', linestyle='-.', linewidth=1, alpha=0.7)
                
                # Добавляем горизонтальную линию на уровне 0
                ax.axhline(y=0, color='gray', linestyle='-', linewidth=0.5, alpha=0.7)
                
                # Проверяем, что correlation_threshold не None перед использованием
                if correlation_threshold is not None:
                    # Добавляем горизонтальную линию порога correlation_threshold
                    threshold_line = ax.axhline(y=correlation_threshold, color='red', linestyle='-', 
                                              linewidth=1.5, alpha=0.8, label=f'Порог: {correlation_threshold}')
                    
                    # Добавляем текстовую метку для порога
                    ax.text(73, correlation_threshold + max_abs_val * 0.03, f"Порог: {correlation_threshold}", 
                            color='red', fontweight='bold', bbox=dict(facecolor='white', alpha=0.7, boxstyle='round'))
                
                # Добавляем точки максимума и минимума
                ...
                # Добавляем текст о разнице между максимумом и минимумом
                ax.text(0.05, 0.70, f'Амплитуда: {max_value - min_value:.0f}',
                        transform=ax.transAxes, fontsize=10, verticalalignment='top',
                        bbox=dict(boxstyle='round', facecolor='lightgreen', alpha=0.6))
                
                # Добавляем значение для точки +45
                if len(correlation_diff) > 145:  # 100 + 45
                    correlation_value = correlation_diff[145]
                    # Добавляем точку и линию для значения +45
                    ax.plot(45, correlation_value, 'o', color='red', markersize=6)
                    
                    # Формируем текст о состоянии счётчика
                    detection_status = f"Счётчик: {correlation_detection_count}/{correlation_detection_threshold}"
                    threshold_status = "ПРЕВЫШЕН" if correlation_value > correlation_threshold else "не превышен"
                    
                    # Определяем цвет блока в зависимости от состояния
                    box_color = 'lightgreen' if correlation_value > correlation_threshold else 'lightcoral'
                    
                    ax.text(0.05, 0.80, f'Значение (+45): {correlation_value:.0f} ({threshold_status})\n{detection_status}',
                            transform=ax.transAxes, fontsize=10, verticalalignment='top',
                            bbox=dict(boxstyle='round', facecolor=box_color, alpha=0.7))
                
                # Проверяем наличие эталонного сигнала и отображаем информацию о различии
                if reference_correlation_diff is not None:
                    pattern_diff = compare_signal_patterns(correlation_diff, reference_correlation_diff)
                    pattern_color = 'lightgreen' if pattern_diff < pattern_difference_threshold else 'lightcoral'
                    
                    ax.text(0.05, 0.7, f'Отличие от эталона: {pattern_diff:.2f}/{pattern_difference_threshold:.2f}',
                            transform=ax.transAxes, fontsize=10, verticalalignment='top',
                            bbox=dict(boxstyle='round', facecolor=pattern_color, alpha=0.7))
                
                # Настраиваем пределы по оси Y (max_abs_val уже определен выше)
                y_min = -max_abs_val * 1.1  # Нижняя граница
                y_max = max_abs_val * 1.1  # Верхняя граница
                ...
                # Проверка, чтобы порог был видимым на графике
                if correlation_threshold is not None:
                    if correlation_threshold > y_max:
                        y_max = correlation_threshold * 1.2
                    elif correlation_threshold < y_min:
                        y_min = correlation_threshold * 1.2
                ...
                # Добавляем легенду с учетом пороговой линии
                if correlation_threshold is not None:
                    handles = [diff_line, threshold_line]
                    labels = ['Разница корреляций', f'Порог: {correlation_threshold}']
                    ax.legend(handles=handles, labels=labels, loc='upper right')
                else:
                    ax.legend([diff_line], ['Разница корреляций'], loc='upper right')
