        ...
        if 10 > start_adr and count_start != 0 and count_sample != count_start and len(dataADC1) > CHUNK:                               # количество данных больше чем одно считывание
            ...
            if count_time_detect > 10: 
                if flag_det_tag > 0:   # первые 10 сек не детектировать 
                    # Метка найдена
                    count_delay_tag += 1
                    print(' ',marked_tau, count_delay_tag, 'Метка определена с максимальным уровнем сигнала:', dataINP_max, 'Порог:', trigger_treshold + (int(count_delay_tag / 100) * 10000), ' !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!', end='\r')
                    ...
                    if count_delay_tag >0:
                        print(' ',marked_tau, count_delay_tag, 'Метки нет. Максимальный уровень сигнала:', dataINP_max, 'Порог:', trigger_treshold + (int(count_delay_tag / 100) * 10000), ' -----------------------------------------------------------', end='\r')
                    count_delay_tag = 0
            else:
