...
def serial_reader():
    ...
    while True:
        data = ser.read_until(serial_protocol_ld2450.REPORT_TAIL)
        data_queue.put(data)
...
def update_plot(frame):
    ...
    while not data_queue.empty():
        ...
        if serial_protocol_ld2450.REPORT_HEADER in serial_protocol_line and serial_protocol_ld2450.REPORT_TAIL in serial_protocol_line:
            # Extract the target values
            all_target_values = serial_protocol_ld2450.read_radar_data(serial_protocol_line)    
...
ax.set_xlim(-3000, 3000)  # Adjust the limits based on your scenario
ax.set_ylim(-6000, 0)
...
plt.xlabel('x [mm]')
plt.ylabel('y [mm]')
fig.canvas.manager.set_window_title("Местоположение обьектов: " + __file__.split('\\')[-1])  # Заголовок окна
plt.grid()
plt.show()
