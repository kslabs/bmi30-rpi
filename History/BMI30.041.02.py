...
count_phase = 3                 # Увеличиваем требуемое количество последовательных обнаружений для включения сигнала
stable_phase_threshold = 2      # Порог стабильности фазы (±5 отсчетов)
...
threshold_margin = 1.2         # Множитель для порога (30% запас)
...
increase_amount = 1 # Сколько увеличивать порог при каждом срабатывании
...
# Добавьте после других глобальных переменных (примерно строка 184)
...
try:
    ...
    while True:
        ...
        if stabilization_cycles < STABILIZATION_LIMIT:
            ...
            # После периода стабилизации применяем обычную логику адаптации
            ...
            # Добавляем новое условие для "слепой зоны" с адаптивным увеличением
            elif max_amp_15 > 0 and max_amp_15 * threshold_margin >= adaptive_threshold and max_amp_15 <= adaptive_threshold:
                # Сигнал в "слепой зоне" - повышаем порог адаптивно
                
                # Рассчитываем разницу между (max_amp_15 * threshold_margin) и adaptive_threshold
                blind_zone_diff = (max_amp_15 * threshold_margin) - adaptive_threshold
                
                # Определяем скорость увеличения в зависимости от разницы
                if blind_zone_diff > 10000:
                    # Большая разница - быстрое увеличение
                    increase_amount = 100
                elif blind_zone_diff > 1000:
                    # Средняя разница - среднее увеличение
                    increase_amount = 10
                else:
                    # Небольшая разница - медленное увеличение
                    increase_amount = 1
                ...
                #if debug_output:
                #    print(f"Сигнал в пограничной зоне: {max_amp_15:.0f}*{threshold_margin}={max_amp_15*threshold_margin:.0f} > {prev_threshold:.0f}, порог +{increase_amount} → {adaptive_threshold:.0f}")
                
                # Продолжаем адаптацию и в этом случае
                below_threshold_counter = 0
                ...
                if threshold_update_enabled:
                    ...
                    if max_amp_15 > 0:  # Если есть какой-то сигнал
                        ...
                        if new_threshold > adaptive_threshold:
                            ...
                            if debug_output and (adaptive_threshold % 100 == 0):
        ...
        # Теперь условие обнаружения сигнала с обновленным адаптивным порогом
        if max_amp_15 > adaptive_threshold:
            ...
            if abs(max_phase_15 - prev_max_phase_15) <= stable_phase_threshold:
                ...
                if consecutive_detections >= count_phase:
                    ...
                    print(f"Сигнал обнаружен! Амплитуда: {max_amp_15:.0f}; Порог: {adaptive_threshold:.0f}; Фаза: {max_phase_15}; Δфазы: {last_phase_diff:.0f};")
        ...
        # Исправить вывод отладочной информации - перенести его ПОСЛЕ обновления счетчика
        if debug_output:
            print(f"Амплитуда: {max_amp_15:.0f}; Адапт.порог: {adaptive_threshold:.0f}; Фаза: {max_phase_15:.0f}; Изменение фазы: {abs(max_phase_15 - prev_max_phase_15):.0f}; Порог стабильности: {stable_phase_threshold}; Последовательных обнаружений: {consecutive_detections}; {'~' if threshold_update_enabled else '-'}; {max_amp_15:.0f}*{threshold_margin}={max_amp_15*threshold_margin:.0f} > {prev_threshold:.0f}, порог +{increase_amount} → {adaptive_threshold:.0f}")
        ...
        if current_time - last_threshold_save_time >= THRESHOLD_SAVE_INTERVAL:
            ...
            if debug_output:
                print(f"!!!!!!!!!!!!!!!!!!!!!!!!!!!Адаптивные пороги сохранены: {adaptive_threshold:.0f}, max_amp_15={max_amp_15:.0f}, max_phase_15={max_phase_15}")
