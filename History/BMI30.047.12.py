...
version = 47.12  # Обновляем версию
...
def sum_zones_with_shifts(data, zone_marks, zone_length=240, shifts_range=10):
    """
    Суммирует зоны данных с указанными начальными индексами с учетом смещений от -10 до +10
    ...
    # Для каждой зоны в списке маркеров
    for start_idx in zone_marks:
        if start_idx + zone_length <= len(data):
            # Извлекаем зону данных
            zone = data[start_idx:start_idx + zone_length]
            
            # Добавляем зону с различными смещениями от -10 до +10
            for shift in range(-shifts_range, shifts_range + 1):
                # Создаем массив для смещенной зоны
                shifted_zone = np.zeros(zone_length)
                
                if shift < 0:
                    # Смещение влево
                    shifted_zone[:zone_length+shift] = zone[-shift:]
                elif shift > 0:
                    # Смещение вправо
                    shifted_zone[shift:] = zone[:zone_length-shift]
                else:
                    # Без смещения
                    shifted_zone = zone
                
                # Добавляем смещенную зону к сумме
                sum_result += shifted_zone
...
def audio_callback(in_data, frame_count, time_info, status):
    ...
    if len(dataADC_L1) > CHUNK:
        ...
        if len(dataADC_P1) > CHUNK and len(dataADC_L1) > CHUNK:
            ...
            # Вычисляем суммы зон с учетом смещений
            sum_high_level_shifted = sum_zones_with_shifts(dataADC_L1, high_level_mark, 240, 10)
            sum_low_level_shifted = sum_zones_with_shifts(dataADC_L1, low_level_mark, 240, 10)
