    ...
    if show_plot:
        ...
        if current_oscilloscope == 1:
            ...
            try:
                ...
                for i in range(12):
                    # Зона 1 (сдвиг вверх)
                    start_idx1 = 350 + i * period  # Для нижней антенны индекс 350 вместо 365
                    if start_idx1 + 200 <= len(dataADC_L1):
                        # Берем данные нижней антенны
                        data1 = dataADC_L1[start_idx1:start_idx1 + 200].copy()
                        # Центрируем с полной компенсацией формы
                        if len(dc_L1_1) == 200:
                    ...
                    # Зона 2 (сдвиг вниз)
                    start_idx2 = 1320 + i * period
                    if start_idx2 + 200 <= len(dataADC_L1):
                        # Берем данные нижней антенны
                        data2 = dataADC_L1[start_idx2:start_idx2 + 200].copy()
                        # Центрируем с полной компенсацией формы
                        if len(dc_L1_2) == 200:
