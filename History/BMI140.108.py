...
CHUNK_out = 1920               # Количество выводимых на экран семплов: 1920-1 период; 2880-1.5 периода; 3840-2 периода;
...
def callback(in_data, frame_count, time_info, status):
    ...
    if len(dataADC1) > CHUNK3:                               # количество данных больше чем одно считывание
        for i in range(0, CHUNK):
            if flag_synchro or (dataADC1[i] < 7500 and dataADC1[i+1] > 7500 and dataADC1[i+31] < -3000):
                flag_synchro = 1
                if dataADC1[i] > ADCmax:
                    ...
                    start_adr = i - 2
                    if i > 3:
                        del dataADC1[0:CHUNK + int((start_adr + 1)/8)]
                    else:
                        del dataADC1[0:CHUNK - int((start_adr - 1)/8)]
                    break 
    ...
    if start_adr < 100:
        ...
        count_test = 0
    print(start_adr, len(dataADC1), count_test)    
...
pwm1 = HardwarePWM(pwm_channel=2, hz=200.009, chip=2)    # pwm_channel 1: 3-GPIO_18; 2-GPIO_19; 
...
#    p.terminate()
    ...
    while stream.is_active():
        ...
        if count_sample != len(dataADC1) and len(dataADC1) > CHUNK:                               # количество данных больше чем одно считывание
        #if 1==1:
            count_sample = len(dataADC1)
            #led_line.set_value(1)
                 
            for i in range(0, CHUNK):                      # находим начало цикла 12 периодов первому встреченному переходу меньше/больше нуля
                if 1==0 and dataADC1[i] < 7000 and dataADC1[i+1] > 7000 and dataADC1[i+36] < -5000: 
                    start_adr = i
                    break
                ...
                if 1==1:
                    if dataADC1[i] > 0: 
                        ...
                        if dataADC1[i] > dataDC[i]:
                            ...
                            dataDC[i] = dataDC[i] - tau
                        dataINP1[i] = (dataADC1[i] - dataDC[i]) * 5
                    else:
            ...
            if waveform_output_display == 2:                         # отображаем осциллограмму входного сигнала с компенсированным DC
                ...
                dataINP1[start_adr+401] = dataINP1[start_adr+401] + 5000
                dataINP1[start_adr+880] = dataINP1[start_adr+883] + 5000
                dataINP1[start_adr+1364] = dataINP1[start_adr+1364] + 5000
                dataINP1[start_adr+1364] = dataINP1[start_adr+1361] + 5000
                dataINP1[start_adr+2321] = dataINP1[start_adr+2321] + 5000
        ...
        if time_wr != int(time.time() / 180):
            time_wr = int(time.time() / 180)
