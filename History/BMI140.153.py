...
153:17.12.2024 решаем проблему оптимизации потока. Не работает синхронизация
...
start_adress = start_adr + 320
stop_adress = 240 + start_adress
tau_start_adr = 3
#
HISTORY_SIZE = 7            # количество исторических массивов
history = np.zeros((HISTORY_SIZE, CHUNK2), dtype=np.float32)    # Массив с историями 

def callback(in_data, frame_count, time_info, status):
    led_line.set_value(1)
    global history, data_to_sum_p, data_to_sum_n, start_adr, count_start, tau_start_adr

    # Преобразуем входные данные
    #dataInt = list(struct.unpack(str(CHUNK * CHANNELS) + 'h', in_data))
    #dataInt1 = dataInt[0::2]  # левый канал
    #dataInt2 = dataInt[1::2]  # правый канал
    
    history[1:] = history[:-1]                              # сдвигаем историю повех существующих элементов массива,освобождаем элемент 0
    #history[0][]
    history[0] = np.frombuffer(in_data, dtype=np.int16)     # обновляем новыми входными данными

    # синхронизация (находим переход от ниже 7000 до выше и проверяем что через 24 семпла уровень выше 6000
    try:
        #if len(history[0]) > CHUNK:    
        sync_mask = (history[0][:CHUNK_out2] > 6000) & (history[0][1:CHUNK_out2-23] > 7000) & (history[0][24:CHUNK_out2] > 6000)
        first_sync_index = np.argmax(sync_mask) if np.any(sync_mask) else -1    # находим номер первого семпла, соответствующего условиям
        
        #sync_indices = np.where(sync_mask)[0][:CHUNK_out2]          # находим номер первого семпла, соответствующего условиям

        if first_sync_index > -1:
            start_adr = first_sync_index                 # первый индекс синхронизации
            history[0] = history[0][start_adr:start_adr +1 ]         # обрезаем массив начиная со стартового адреса. удаляем ненужные данные до точки синхронизации
        #else:
        #    sync_mask = (False)
        #    sync_indices = 0
        #    has_true_value = 0
    except Exception as e:
        print(f"1. данные не готовы!: {repr(e)}", e)
        sync_mask = (False)
        sync_indices = 0
            
            
    print(CHUNK, len(history[0]), np.max(history[0]), len(sync_indices))
    
    # проверка синхронизации
    if start_adr < 10 and tau_start_adr == 0:
        count_start += 1

        # разворачиваем историю в массив
        dataADC = history.flatten(order='C')
        total_samples = len(dataADC)

        # вычисляем длину сегмента
        segment_length = stop_adress - start_adress
        if segment_length <= 0:
            print(f"ошибка: некоректная длина сегмента segment_length={segment_length}. start_adress - stop_adress!")
            led_line.set_value(0)
            return (in_data, pyaudio.paContinue)

        # вычисление сегментов
        num_segments = total_samples // (1920 * segment_length)

        # добавляем отладочный вывод
        #print(f"данные для анализа: total_samples={total_samples}, segment_length={segment_length}, num_segments={num_segments}")
        
        if num_segments > 0:
            segment_data_len = num_segments * 1920 * segment_length
            
            if segment_data_len <= total_samples:
                reshaped_array = dataADC[start_adress : start_adress + segment_data_len].reshape(num_segments, 1920 * segment_length)

                data_to_sum_p = np.mean(
                    reshaped_array,
                    axis=0
                )
                data_to_sum_n = np.mean(
                    dataADC[start_adress + 960 : start_adress + 960 + segment_data_len].reshape(num_segments, 1920 * segment_length),
                    axis=0
                )

                # убедимся, что массивы соответствуют ожидаемым размерам
                if len(data_to_sum_p) != CHUNK_out2 or len(data_to_sum_n) != CHUNK_out2:
                    print("массивы имеют недостаточный размер CHUNK_out2.")
                    data_to_sum_p = np.zeros(CHUNK_out2)
                    data_to_sum_n = np.zeros(CHUNK_out2)
            else:
                print(f"недостаточно данных для reshaping: {total_samples} данных, требуется {segment_data_len}.")
        #else:
            #print(f"недостаточно сегментов для анализа. total_samples={total_samples}, segment_length={segment_length}, num_segments={num_segments}")
    else:
        count_start = 0
        data_to_sum_p = np.zeros(CHUNK_out2)
        data_to_sum_n = np.zeros(CHUNK_out2)
...
try:
    ...
    dataDC = np.zeros(CHUNK_out2, 'int64') 
...
tau3 = 100
tau4 = 1000
trigger_treshold1 =  500
trigger_treshold2 =  1700
trigger_treshold3 =  4500
...
try:                        # для аккуратного принудительного останова программы с закрытием портов
    ...
    while running and stream.is_active():
        ...
        if start_adr > 9:
            ...
            #dataINP11 = (data_to_sum_n * -500) - dataDC
            ...
            if waveform_output_display == 1:                         # отображаем осциллограмму сырого входного сигнала history[0]
                ...
                dataADC1[start_adr+300] = dataADC1[start_adr+300] + 5000
                ...
                display_line.set_ydata(history[0][0:1920])
                fig.canvas.draw()
            ...
            if waveform_output_display == 6:                         # отображаем результирующую осциллограмму усредненного сигнала 7-х циклов с компенсированным DC
                ...
                if temp < 10000:
                    ...
                    tau = 10
                dataINP_result1[mark12] = dataINP_result[mark12] - 1000
                dataINP_result1[mark13] = dataINP_result[mark13] + 1000
                dataINP_result1[mark14] = dataINP_result[mark14] - 1000
                dataINP_result1[mark15] = dataINP_result[mark15] + 1000
                dataINP_result1[mark16] = dataINP_result[mark16] - 1000
                dataINP_result1[0] = dataINP_result[0] + 1000
