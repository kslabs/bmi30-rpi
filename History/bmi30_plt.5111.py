...
BUTTON_CONFIG = {
    ...
    17: {
        'text': 'DC компоненты суммированных зон',
        'show_plot': True,
        'ylim': (-10000, 10000),  # Устанавливаем значения по умолчанию
        'line1': {'label': 'DC High Level', 'color': 'blue', 'visible': True},
        'line2': {'label': 'DC Low Level', 'color': 'red', 'visible': True},
    },
    18: {
        'text': 'AC компоненты суммированных зон',
        'show_plot': True,
        'ylim': (-10000, 10000),  # Устанавливаем значения по умолчанию
        'line1': {'label': 'AC High Level', 'color': 'blue', 'visible': True},
        'line2': {'label': 'AC Low Level', 'color': 'red', 'visible': True},
    },
    19: {
...
def update_graph(show_plot, update_needed, current_oscilloscope, start_sample, num_samples, 
                ...
                sum_high_level_shifted, sum_low_level_shifted, 
                cross_correlation_sum, ac_high_level, ac_low_level,
                antiphase_correlation_sum,
                dc_high_level, dc_low_level):  # Добавлены новые параметры
    ...
    if show_plot:
        ...
        if current_oscilloscope == 1:
        ...
        elif current_oscilloscope == 17 and show_plot:
            # Очищаем график и настраиваем
            ax.clear()
            ax.set_title('DC компоненты суммированных зон')
            ax.grid(True)
            
            # Отладочный вывод для проверки данных
            print(f"*** График 17: DC компоненты - данные: ***")
            print(f"dc_high_level: тип={type(dc_high_level).__name__}, размер={len(dc_high_level) if dc_high_level is not None else 'None'}")
            print(f"Значения: мин={np.min(dc_high_level) if dc_high_level is not None and len(dc_high_level)>0 else 'N/A'}, "
                  f"макс={np.max(dc_high_level) if dc_high_level is not None and len(dc_high_level)>0 else 'N/A'}")
            
            # Диагностический вывод содержимого массивов
            if dc_high_level is not None and len(dc_high_level) > 0:
                print(f"DC high min={np.min(dc_high_level):.1f}, max={np.max(dc_high_level):.1f}, mean={np.mean(dc_high_level):.1f}")
            
            if dc_low_level is not None and len(dc_low_level) > 0:
                print(f"DC low min={np.min(dc_low_level):.1f}, max={np.max(dc_low_level):.1f}, mean={np.mean(dc_low_level):.1f}")
                
            # Отображаем данные только если они существуют и не пустые
            if (dc_high_level is not None and len(dc_high_level) > 0 and not np.all(dc_high_level == 0)) or \
               (dc_low_level is not None and len(dc_low_level) > 0 and not np.all(dc_low_level == 0)):
                
                # Создаем массив индексов для оси X
                x_indices = np.arange(len(dc_high_level) if dc_high_level is not None and len(dc_high_level) > 0 else len(dc_low_level))
                
                # Рисуем графики
                handles = []
                labels = []
                
                if dc_high_level is not None and len(dc_high_level) > 0 and not np.all(dc_high_level == 0):
                    high_line, = ax.plot(x_indices, dc_high_level, 'b-', linewidth=1.5)
                    high_line.set_label('DC High Level')
                    handles.append(high_line)
                    labels.append('DC High Level')
                    line1.set_data(x_indices, dc_high_level)
                else:
                    line1.set_data([], [])
                
                if dc_low_level is not None and len(dc_low_level) > 0 and not np.all(dc_low_level == 0):
                    low_line, = ax.plot(x_indices, dc_low_level, 'r-', linewidth=1.5)
                    low_line.set_label('DC Low Level')
                    handles.append(low_line)
                    labels.append('DC Low Level')
                    line2.set_data(x_indices, dc_low_level)
                else:
                    line2.set_data([], [])
                ...
                ax.set_ylabel('Амплитуда DC')
                
                # Добавляем легенду если есть линии
                if handles:
                    ax.legend(handles=handles, labels=labels)
            else:
                # Отображаем текст об отсутствии данных
                ax.text(0.5, 0.5, 'DC компоненты отсутствуют', 
                       horizontalalignment='center', verticalalignment='center',
                       transform=ax.transAxes, fontsize=14)
                line1.set_data([], [])
                line2.set_data([], [])
                
                # Устанавливаем безопасные пределы
        ...
        elif current_oscilloscope == 18 and show_plot:
            # Очищаем график
            ax.clear()
            ax.set_title('AC компоненты суммированных зон')
            ax.grid(True)
            ...
            if ac_high_level is not None and ac_low_level is not None and len(ac_high_level) > 0 and len(ac_low_level) > 0:
                ...
                # Безопасно вычисляем пределы оси Y
                max_abs_val = max(np.max(np.abs(ac_high_level)), np.max(np.abs(ac_low_level))) * 1.1 if len(ac_high_level) > 0 and len(ac_low_level) > 0 else 1
                ax.set_ylim(-max_abs_val, max_abs_val)  # Явно устанавливаем пределы
                
                # Добавляем метки осей
                ax.set_xlabel('Отсчеты')
                ax.set_ylabel('Амплитуда AC')
                ...
                # Устанавливаем безопасные пределы осей
                ax.set_ylim(-1, 1)
