...
def update_graph(show_plot, update_needed, current_oscilloscope, start_sample, num_samples, dataADC_P1, dataADC_L1, line1, line2, ax, fig):
    if show_plot:
        ...
        if current_oscilloscope == 1:
            ...
            # Зоны ожидания для верхней антенны на одном графике с новыми значениями
            x_axis = np.arange(200)  # Создаем единую ось X длиной 200 семплов
            zone1 = dataADC_P1[365:565]  # Первая зона: изменено с 350-550 на 365-565
            zone2 = dataADC_P1[1320:1520]  # Вторая зона: без изменений
            line1.set_data(x_axis, zone1)  # Фиолетовый график
            line2.set_data(x_axis, zone2)  # Шоколадный график
            line1.set_label('Зона 1 (365-565)')  # Обновляем метку в легенде
            line2.set_label('Зона 2 (1320-1520)')
            line1.set_color('purple')
            line2.set_color('chocolate')
            ax.set_xlim(0, 200)
        elif current_oscilloscope == 5:
            # Зоны ожидания для нижней антенны на одном графике
            x_axis = np.arange(200)  # Создаем единую ось X длиной 200 семплов
            zone1 = dataADC_L1[350:550]  # Первая зона
            zone2 = dataADC_L1[1320:1520]  # Вторая зона
            line1.set_data(x_axis, zone1)  # Фиолетовый график
            line2.set_data(x_axis, zone2)  # Шоколадный график
