...
try:                        # для аккуратного принудительного останова программы с закрытием портов
    while True:
        ...
        if flag_adc:
            #dataInt = list(struct.unpack(str((CHUNK) * 4) + 'h', pwm_adc_100.start_adc(CHUNK * 2)))
            dataADC1 = pwm_adc_101.start_adc(CHUNK)
            flag_adc = 0
            for i in range(0, int(CHUNK)):                      # находим начало цикла 12 периодов
                if dataADC1[i] < 0 and dataADC1[i+1] > 0:
                    start_adr = i
            ...
            dataADC2 = pwm_adc_101.start_adc(CHUNK)
            flag_adc = 1
            for i in range(0, int(CHUNK)):
                if dataADC2[i] < 0 and dataADC2[i+1] > 0:
                    start_adr = i
                    #stop_adr = int((CHUNK / 2) + i)
                    break
            if waveform_output_display:
                #print(dataInt1)
                display_line.set_ydata(dataADC2[start_adr:(CHUNK + start_adr):])
