...
def callback(in_data, frame_count, time_info, status):
    ...
    adc_L0 = np.frombuffer(in_data, dtype=np.int16)[0::2]     # Левый аудио канал   
    adc_P0 = np.frombuffer(in_data, dtype=np.int16)[1::2]     # Правый аудиоканал

    # синхронизация к каждому периоду
    # создаем логические массивы для условий
    condition1 = adc_P0[:-35] < threshold   # adc_P0[i] < 27000
    condition2 = adc_P0[1:-34] > threshold  # adc_P0[i+1] > 27000
    condition3 = adc_P0[28:-7] > threshold  # adc_P0[i+28] > 27000
    condition4 = adc_P0[35:] < threshold    # adc_P0[i+35] < 27000
    
    # находим все индексы, где условия выполняются
    valid_starts = np.where(condition1 & condition2 & condition3 & condition4)[0]
    ...
    #front_values_array = np.array(front_values_list) if front_values_list else np.empty((0, 240), dtype=int)
    #fall_values_array = np.array(fall_values_list) if fall_values_list else np.empty((0, 240), dtype=int)
    
    #
    #adc_P0f = np.mean(front_values_array, axis=0) if front_values_array.size > 0 else np.zeros(240)
    #adc_P0s = np.mean(spad_values_array, axis=0) if spad_values_array.size > 0 else np.zeros(240)
    
    # вычисление средних значений
    adc_P0f = np.round(np.mean(front_values_array, axis=0)).astype(int) if front_values_array.size > 0 else np.zeros(240, dtype=int)
    adc_P0s = np.round(np.mean(spad_values_array, axis=0)).astype(int) if spad_values_array.size > 0 else np.zeros(240, dtype=int)
