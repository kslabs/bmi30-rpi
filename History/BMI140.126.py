...
def callback(in_data, frame_count, time_info, status):
    ...
    new_data = np.frombuffer(in_data, dtype=np.int16)
    dataADC1 = np.concatenate((dataADC1, new_data))
    dataADC = np.concatenate((dataADC3, dataADC2, dataADC1))
    
    #print(start_adr,  len(dataADC), len(dataADC1), len(dataADC2),len(dataADC3), count_start)
...
#    p.terminate()
    ...
    while stream.is_active():
        ...
        if 10 > start_adr and count_start != 0 and count_sample != count_start and len(dataADC1) > CHUNK:                               # количество данных больше чем одно считывание
            ...
            if dataINP_max > trigger_treshold + (int(count_delay_tag / 100) * 10000):
                flag_det_tag += 1
                marked_tau = '+'
            else:
            ...
            if count_time_detect > 10: 
                if flag_det_tag > 0:   # первые 10 сек не детектировать 
                    # Метка найдена
                    count_delay_tag += 1
                    print(marked_tau, count_delay_tag, 'Метка определена с максимальным уровнем сигнала:', dataINP_max, ' !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!')
                    ...
                    if count_delay_tag >0:
                        print(marked_tau, count_delay_tag, 'Метки нет. Максимальный уровень сигнала:', dataINP_max, ' -----------------------------------------------------------')
                    count_delay_tag = 0
            else:
