...
waveform_output_display = 6     # Разрешение вывода на экран: 0-без вывода; 1-сырого сигнала; 2-сигнала без постоянной; 3-постоянная составляющая сигнала
                                # 4-сумма по фронту; 5-сумма по спаду; 6-сумма 7 циклов без DC; 7- свертка пополам 6-го графика; 8-вторая производная; 9-тест
CHUNK_start_out = 0             # стартовый номер семпла выводимых на экран
CHUNK_out1 = 220               # Количество выводимых на экран семплов: 1920-1 период; 2880-1.5 периода; 3840-2 периода;
...
def callback(in_data, frame_count, time_info, status):
    ...
    if len(dataADC1) > CHUNK:
        ...
        for i in range(0, CHUNK):
            if flag_synchro or (dataADC1[i] < 7000 and dataADC1[i+1] > 7000 and dataADC1[i+28] > 7000 and dataADC1[i+35] < 7000):
                flag_synchro = 1
...
tau3 = 100
tau4 = 1000
trigger_treshold1 =  800
trigger_treshold2 =  2600
trigger_treshold3 =  6000
...
try:                        # для аккуратного принудительного останова программы с закрытием портов
    ...
    while running and stream.is_active():
        ...
        if start_adr > 9:
            ...
            data_to_sum_p2 = (data_to_sum_p * 500) -dataDC_p
            data_to_sum_n2 = (data_to_sum_n * 500) -dataDC_n
            data_to_sum_p1 = data_to_sum_p2 - np.mean(data_to_sum_p2)
            data_to_sum_n1 = data_to_sum_n2 - np.mean(data_to_sum_n2)
                
            # Компенсируем среднюю составляющую, что бы осциллограмма не прыгала вверх-вниз
            dataINP12 = dataINP11 - np.mean(dataINP11)
            # Компенсируем наклоны осциллограммы, что бы осциллограмма не качалась как качели
            dataINP_result = ((dataINP_result + align_to_center(dataINP12)) // 2).astype(int)
            ...
            if waveform_output_display == 1:                         # отображаем осциллограмму сырого входного сигнала
                ...
                dataADC1[start_adr] = dataADC1[start_adr] + 5000
                dataADC1[start_adr+30] = dataADC1[start_adr+30] + 5000
                dataADC1[start_adr+25] = dataADC1[start_adr+25] - 5000
            ...
            if waveform_output_display == 10:                         # отображаем осциллограмму сырого входного сигнала
                display_line.set_ydata(new_data[CHUNK_start_out:CHUNK_out1+CHUNK_start_out:])
                fig.canvas.draw()
                fig.canvas.flush_events()

            if waveform_output_display == 14:                         # отображаем осциллограмму DC

                line_p.set_ydata(data_to_sum_p1)  # График массива data_to_sum_p
                line_n.set_ydata(data_to_sum_n1)  # График массива data_to_sum_n
