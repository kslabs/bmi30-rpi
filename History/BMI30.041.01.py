...
try:
    ...
    while True:
        ...
        # Заменяем блок логики адаптивного порога на этот код:
        ...
        if debug_output:
            info_str = (
            f"Сигнал: {max_amp_15:6.0f} | "
            f"А.Порог: {prev_threshold:.0f} → {adaptive_threshold:6.0f} | "
            f"Детекц: {consecutive_detections}/{count_phase} | "
            f"Фаза:  Δ{abs(max_phase_15 - prev_max_phase_15):3} | "
            f"Порог стаб.: {stable_phase_threshold} | "
            f" {consecutive_detections}; {'~' if threshold_update_enabled else '-'} | "
            f"Порог: {'↓' if adaptive_threshold > prev_threshold else '↑'} {abs(adaptive_threshold - prev_threshold):6.0f} "
            )
            print(info_str)
