...
from bmi30_def import initialize_gpio, set_resistor, toggle_gpio6_and_update_resistors, update_plot, update_start
from bmi30_plt import initialize_plot, initialize_sliders  # Импортируем функцию initialize_sliders
...
def audio_callback(in_data, frame_count, time_info, status):
    ...
    dataADC1 = np.concatenate((dataADC1[-CHUNK:], new_data[:, 0]))[-CHUNK:]  # Ограничиваем размер
    dataADC2 = np.concatenate((dataADC2[-CHUNK:], new_data[:, 1]))[-CHUNK:]  # Ограничиваем размер
...
start_sample_slider, num_samples_slider = initialize_sliders(fig, ax, CHUNK, start_sample, num_samples, 
                                                             lambda val: update_start(val, start_sample_slider, num_samples_slider, dataADC1, dataADC2, line1, line2, ax, CHUNK),
                                                             lambda val: update_plot(val, start_sample_slider, num_samples_slider, dataADC1, dataADC2, line1, line2, ax, CHUNK))
...
button0.on_clicked(lambda event: switch_graph('0'))
button1.on_clicked(lambda event: switch_graph('1'))
button2.on_clicked(lambda event: switch_graph('2'))
button3.on_clicked(lambda event: switch_graph('3'))
button4.on_clicked(lambda event: switch_graph('4'))
button5.on_clicked(lambda event: switch_graph('5'))
button6.on_clicked(lambda event: switch_graph('6'))
button7.on_clicked(lambda event: switch_graph('7'))
button8.on_clicked(lambda event: switch_graph('8'))
button9.on_clicked(lambda event: switch_graph('9'))
...
try:
    ...
    while True:
        if show_plot[0] and update_needed[0]:
            if current_oscilloscope[0] == 1:
                ...
                ax.relim()
                ax.autoscale_view()
            update_needed[0] = False  # Сбрасываем флаг после обновления графика
