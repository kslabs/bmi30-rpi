...
version = 51.11  # Обновляем версию
...
def load_dc_zones_components():
    """Загружает DC компоненты зон из файла"""
    global dc_high_level, dc_low_level
    try:
        with open('dc_zones_components.json', 'r') as f:
            dc_data = json.load(f)
            dc_high_level = np.array(dc_data['dc_high_level'])
            dc_low_level = np.array(dc_data['dc_low_level'])
            print(f"DC компоненты зон загружены (сохранены: {dc_data['timestamp']})")
    except FileNotFoundError:
        print("Файл с DC компонентами зон не найден, используются нулевые значения")
    except Exception as e:
        print(f"Ошибка при загрузке DC компонент зон: {e}")
...
# Добавьте эту функцию сразу после функции load_dc_zones_components()

def save_dc_zones_components():
    """Сохраняет DC компоненты зон в файл"""
    global dc_high_level, dc_low_level
    
    dc_data = {
        'dc_high_level': dc_high_level.tolist(),
        'dc_low_level': dc_low_level.tolist(),
        'timestamp': datetime.now().strftime("%Y-%m-%d %H:%M:%S")
    }
    
    try:
        with open('dc_zones_components.json', 'w') as f:
            json.dump(dc_data, f)
        print(f"DC компоненты зон сохранены: {datetime.now().strftime('%H:%M:%S')}")
    except Exception as e:
        print(f"Ошибка при сохранении DC компонент зон: {e}")
...
def normalize_periods(periods):
    """
    Нормализует периоды сигнала, удаляя наклон и центрируя относительно нуля
    
    Args:
        periods: Массив периодов сигнала
        
    Returns:
        Нормализованные периоды
    """
    ...
    # Для каждого периода убираем наклон и центрируем относительно нуля
...
def adapt_dc_component(data, dc_data, counter, prev_direction):
    """
    Адаптирует DC компоненту на основе текущих данных с учетом счетчика изменений
    """
    # Защита от пустых данных и принудительная инициализация
    if data is None or len(data) == 0:
        print("ОШИБКА: Пустые входные данные в adapt_dc_component")
        return dc_data, counter, prev_direction
        
    if dc_data is None or len(dc_data) == 0:
        print("ОШИБКА: dc_data пустой, создаем новый массив")
        dc_data = np.ones_like(data) * 1000  # Инициализируем ненулевыми значениями
        
    # Проверяем наличие ненулевых значений
    if np.all(dc_data == 0):
        print("ВНИМАНИЕ: dc_data содержит только нули, инициализируем")
        dc_data = np.ones_like(data) * 1000  # Инициализируем ненулевыми значениями
    
    # Более агрессивные шаги изменения для быстрого накопления DC
    current_direction = data > dc_data
    ...
    # Увеличим шаг в 5 раз для ускорения адаптации
    #step = np.maximum(1, 2 ** (counter // 5))  # Уменьшаем порог с 10 до 5 и увеличиваем базовый шаг
    step = np.maximum(1, 2 * (counter // 10))  # Уменьшаем порог с 10 до 5 и увеличиваем базовый шаг
    #step = 1
    
    # Обновляем DC компоненту
    dc_data[current_direction] += step[current_direction]
    dc_data[~current_direction] -= step[~current_direction]
    
    # Отладочный вывод каждые 10 вызовов
    if np.random.random() < 0.1:  # 10% вероятность вывода
        print(f"DC компоненты - min: {np.min(dc_data):.1f}, max: {np.max(dc_data):.1f}, mean: {np.mean(dc_data):.1f}")
...
# DC компоненты для суммированных зон
dc_high_level = np.zeros(240)  # DC компонента high_level зон
dc_low_level = np.zeros(240)   # DC компонента low_level зон

# Счетчики последовательных изменений в одном направлении для каждого семпла
dc_counter_high = np.ones(240)  # Счетчики для high_level
dc_counter_low = np.ones(240)   # Счетчики для low_level

# Предыдущие направления изменения (True - увеличение, False - уменьшение)
prev_dir_high = np.zeros(240, dtype=bool)  # Направления для high_level
prev_dir_low = np.zeros(240, dtype=bool)   # Направления для low_level

# Интервал сохранения DC компонент (в секундах)
DC_ZONES_SAVE_INTERVAL = 180  # 3 минуты
last_dc_zones_save_time = time.time()
...
def audio_callback(in_data, frame_count, time_info, status):
    ...
    global dc_high_level, dc_counter_high, prev_dir_high, dc_low_level, dc_counter_low, prev_dir_low
    global ac_high_level, ac_low_level
    global last_dc_zones_save_time  # Добавляем эту переменную

    # Принудительная инициализация DC компонент, если они отсутствуют
    if dc_high_level is None or np.all(dc_high_level == 0):
        print("Принудительная инициализация dc_high_level со стартовыми значениями")
        dc_high_level = np.full(240, 1000.0)  # Инициализируем ненулевыми значениями
    
    if dc_low_level is None or np.all(dc_low_level == 0):
        print("Принудительная инициализация dc_low_level со стартовыми значениями")
        dc_low_level = np.full(240, 1000.0)  # Инициализируем ненулевыми значениями
    ...
    if len(dataADC_L1) > CHUNK:
        ...
        if len(dataADC_P1) > CHUNK and len(dataADC_L1) > CHUNK:
            ...
            dc_P1_1, dc_counter_P1_1, prev_direction_P1_1 = adapt_dc_component(
                zone_P1_1, dc_P1_1, dc_counter_P1_1, prev_direction_P1_1
            )
            dc_P1_2, dc_counter_P1_2, prev_direction_P1_2 = adapt_dc_component(
                zone_P1_2, dc_P1_2, dc_counter_P1_2, prev_direction_P1_2
            )
            dc_L1_1, dc_counter_L1_1, prev_direction_L1_1 = adapt_dc_component(
                zone_L1_1, dc_L1_1, dc_counter_L1_1, prev_direction_L1_1
            )
            dc_L1_2, dc_counter_L1_2, prev_direction_L1_2 = adapt_dc_component(
            ...
            # После вычисления sum_high_level_shifted и sum_low_level_shifted:

            # Проверяем и инициализируем массивы, если они не существуют или имеют неправильный размер
            if dc_high_level is None or len(dc_high_level) != len(sum_high_level_shifted):
                print(f"Переинициализация dc_high_level с размером {len(sum_high_level_shifted)}")
                dc_high_level = np.zeros_like(sum_high_level_shifted)
                
            if dc_low_level is None or len(dc_low_level) != len(sum_low_level_shifted):
                print(f"Переинициализация dc_low_level с размером {len(sum_low_level_shifted)}")
                dc_low_level = np.zeros_like(sum_low_level_shifted)

            if dc_counter_high is None or len(dc_counter_high) != len(sum_high_level_shifted):
                dc_counter_high = np.ones_like(sum_high_level_shifted)
                
            if dc_counter_low is None or len(dc_counter_low) != len(sum_low_level_shifted):
                dc_counter_low = np.ones_like(sum_low_level_shifted)
                
            if prev_dir_high is None or len(prev_dir_high) != len(sum_high_level_shifted):
                prev_dir_high = np.zeros_like(sum_high_level_shifted, dtype=bool)
                
            if prev_dir_low is None or len(prev_dir_low) != len(sum_low_level_shifted):
                prev_dir_low = np.zeros_like(sum_low_level_shifted, dtype=bool)

            # Адаптируем DC компоненты для зон
            dc_high_level, dc_counter_high, prev_dir_high = adapt_dc_component(
                sum_high_level_shifted, dc_high_level, dc_counter_high, prev_dir_high)
            dc_low_level, dc_counter_low, prev_dir_low = adapt_dc_component(
                sum_low_level_shifted, dc_low_level, dc_counter_low, prev_dir_low)

            # Диагностический вывод (раз в 100 вызовов)
            if np.random.random() < 0.01:  # 1% вероятность вывода
                print(f"DC компоненты после адаптации:")
                print(f"  - dc_high_level: min={np.min(dc_high_level):.1f}, max={np.max(dc_high_level):.1f}, mean={np.mean(dc_high_level):.1f}")
                print(f"  - dc_low_level: min={np.min(dc_low_level):.1f}, max={np.max(dc_low_level):.1f}, mean={np.mean(dc_low_level):.1f}")
                print(f"  - sum_high_level_shifted: min={np.min(sum_high_level_shifted):.1f}, max={np.max(sum_high_level_shifted):.1f}")

            # Вычисляем AC компоненты для зон (вычитаем DC)
            ac_high_level = sum_high_level_shifted - dc_high_level
            ac_low_level = sum_low_level_shifted - dc_low_level

            # Вычисляем кросс-корреляцию между уже AC компонентами
            cross_correlation_sum = calculate_cross_correlation(
                ac_high_level, ac_low_level, CROSS_CORRELATION_SHIFTS)

            # Вычисляем противофазную корреляцию между AC компонентами
        ...
        if current_time - last_dc_save_time >= DC_SAVE_INTERVAL:
            ...
            print(f"++++++++++++++++ 1.DC компоненты сохранены: {datetime.now().strftime('%H:%M:%S')}")

        # В основном цикле программы, проверка необходимости сохранения DC компонент зон:
        current_time = time.time()
        if current_time - last_dc_zones_save_time >= DC_ZONES_SAVE_INTERVAL:
            save_dc_zones_components()
            last_dc_zones_save_time = current_time
            print(f"++++++++++++++++!!!!!!!! 2.DC компоненты сохранены: {datetime.now().strftime('%H:%M:%S')}")
...
# После других инициализаций в начале программы:
load_dc_zones_components()
...
try:
    ...
    while True:
        ...
        continue_loop, update_needed = update_graph(
            ...
            # DC компоненты для high и low зон
            dc_high_level,
            dc_low_level
        )
