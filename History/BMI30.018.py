...
version = 17.00  # Обновляем версию
...
DC_THRESHOLD_HIGH = 20000  # Порог для больших отклонений
DC_THRESHOLD_LOW = 5000    # Порог для малых отклонений
    ...
    if len(dataADC_L1) > CHUNK:
        ...
        if len(dataADC_P1) > CHUNK and len(dataADC_L1) > CHUNK:
            ...
            for j in range(200):
                ...
                diff = abs(zone_P1_1[j] - dc_P1_1[j])
                current_direction = dc_P1_1[j] > zone_P1_1[j]
                
                if current_direction == prev_direction_P1_1[j]:
                    dc_counter_P1_1[j] += 1
                else:
                    dc_counter_P1_1[j] = 1
                    prev_direction_P1_1[j] = current_direction

                if diff > DC_THRESHOLD_HIGH:
                    step = DC_STEP_LARGE
                elif diff > DC_THRESHOLD_LOW:
                    step = DC_STEP_MEDIUM
                else:
                    step = DC_STEP_SMALL
                
                step = step * dc_counter_P1_1[j]  # Умножаем шаг на счетчик
                ...
                diff = abs(zone_P1_2[j] - dc_P1_2[j])
                current_direction = dc_P1_2[j] > zone_P1_2[j]
                
                if current_direction == prev_direction_P1_2[j]:
                    dc_counter_P1_2[j] += 1
                else:
                    dc_counter_P1_2[j] = 1
                    prev_direction_P1_2[j] = current_direction
                ...
                step = step * dc_counter_P1_2[j]  # Умножаем шаг на счетчик
                ...
                # Аналогично для нижней антенны зона 1
                diff = abs(zone_L1_1[j] - dc_L1_1[j])
                current_direction = dc_L1_1[j] > zone_L1_1[j]
                
                if current_direction == prev_direction_L1_1[j]:
                    dc_counter_L1_1[j] += 1
                else:
                    dc_counter_L1_1[j] = 1
                    prev_direction_L1_1[j] = current_direction
                ...
                step = step * dc_counter_L1_1[j]  # Умножаем шаг на счетчик
                ...
                diff = abs(zone_L1_2[j] - dc_L1_2[j])
                current_direction = dc_L1_2[j] > zone_L1_2[j]
                
                if current_direction == prev_direction_L1_2[j]:
                    dc_counter_L1_2[j] += 1
                else:
                    dc_counter_L1_2[j] = 1
                    prev_direction_L1_2[j] = current_direction
                ...
                step = step * dc_counter_L1_2[j]  # Умножаем шаг на счетчик
...
def handle_pwm_interrupt():
    ...
    if interrupt_counter == 12:  # На 12-м прерывании
        ...
        gpio1.on()  # Возвращаем 1
        interrupt_counter = 0  # Сбрасываем счетчик
        
# Привязываем обработчик к прерыванию по фронту
gpio26.when_pressed = handle_pwm_interrupt

# Оставить эту привязку обработчика
gpio26.when_pressed = handle_pwm_interrupt
