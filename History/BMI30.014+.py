...
version = 14.00  # Обновляем версию
...
def audio_callback(in_data, frame_count, time_info, status):
    ...
    if len(dataADC_L1) > CHUNK:
        ...
        # Сохраняем зоны ожидания после синхронизации
        if len(dataADC_P1) >= 1520 and len(dataADC_L1) >= 1520:
            # Зоны для верхней антенны
            zone_P1_1 = dataADC_P1[365:565].copy()  # Первая зона: 365-565
            zone_P1_2 = dataADC_P1[1320:1520].copy()  # Вторая зона
            
            # Зоны для нижней антенны
            zone_L1_1 = dataADC_L1[350:550].copy()  # Первая зона
            zone_L1_2 = dataADC_L1[1320:1520].copy()  # Вторая зона
