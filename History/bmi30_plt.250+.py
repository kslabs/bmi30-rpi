    ...
    if show_plot:
        ...
        if current_oscilloscope == 1:
            ...
            try:
                ...
                # Упрощаем - отображаем только 6 периодов вместо 12
                period = 1920
                # Используем только 6 разных цветов
                ...
                # Отображаем только 6 периодов для обеих зон
                for i in range(6):
                    # Зона 1 (сдвиг вверх)
                    ...
                    if start_idx1 + 200 <= len(dataADC_P1):
                        ...
                        legend_labels.append(f'Период {i+1}')
