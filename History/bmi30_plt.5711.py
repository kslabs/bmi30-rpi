    ...
    if show_plot:
        ...
        if current_oscilloscope == 1:
            ...
            if 'correlation_dc_component' in globals() and correlation_dc_component is not None and len(correlation_dc_component) > 0:
                ...
                dc_line, = ax.plot(shifts, correlation_dc_component, 'r-', linewidth=2, label='DC корреляции')
                ...
                if cross_correlation_sum is not None and antiphase_correlation_sum is not None:
                    ...
                    diff_line, = ax.plot(shifts, correlation_diff, 'b-', linewidth=1, alpha=0.6, label='Текущая разница')
                    ...
                    if len(correlation_dc_component) == len(correlation_diff):
                        ...
                        ac_line, = ax.plot(shifts, correlation_ac, 'g-', linewidth=1, label='AC компонента')
                ...
                # Если DC-компоненты нет - очищаем график
                ax.clear()
                ax.set_title('DC-компонента графика разности корреляций')
                ax.set_xlabel('Смещение (семплы)')
                ax.set_ylabel('Амплитуда')
...
def compute_correlation_dc(correlation_diff, alpha=0.01):
    """
    Медленное вычисление DC-составляющей для графика разности корреляций
    
    Args:
        correlation_diff: Массив разности корреляций
        alpha: Коэффициент для медленного накопления (меньше - медленнее)
        
    Returns:
        DC-компонента графика разности корреляций
    """
    # Получаем глобальную DC-компоненту или создаем новую
    global correlation_dc_component
    
    if 'correlation_dc_component' not in globals() or correlation_dc_component is None or len(correlation_dc_component) != len(correlation_diff):
        correlation_dc_component = np.zeros_like(correlation_diff)
