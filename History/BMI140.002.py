...
CHUNK = 32768       # Устанавливаем количество семлов считывания  (46080 расчетное)  32765
pwm_adc.conf(CHUNK)
...
try:                        # для аккуратного принудительного останова программы с закрытием портов
    while True:
        if allowing_radar_rule:
            ...
            if time.time() - timer < 10:	# Управляем светодиодом / генератором передатчика
                ...
                on_off_pwm = 1
                pwm_adc.start_pwm()			# Включаем передачу PWM
            else:
                led_line.set_value(1)
                timerP = "~"
                on_off_pwm = 0
                pwm_adc.stop_pwm()			# Выключаем передачу PWM
            ...
            pwm_adc.start_pwm()			# Включаем передачу
        ...
        dataInt = list(struct.unpack(str(CHUNK * 2) + 'h', pwm_adc.start_adc(CHUNK)))
        dataInt1 = dataInt[0::2]  # разделяем стереоканалы, это левый канал с синхросигналом первая осциллограмма
        #dataInt2 = dataInt[1::2]  # разделяем стереоканалы, это правый канал с входными данными втроая осциллограмма
        ...
        if waveform_output_display:
            #print(dataInt1)
            display_line.set_ydata(dataInt1) 

            fig.canvas.draw()
            fig.canvas.flush_events()
except Exception as e:
    print(f" Ошибка: {repr(e)}", e)
    ...
    led_line.set_value(1)
    pwm_adc.stop_pwm()
    print("\n\nUART порт закрыт и светодиод/pwm выключен")
