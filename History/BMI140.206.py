...
accumulation_limit = 1000       # Максимальное число накоплений 

# Аккумуляторы для накопления значений
adc_P0f_sum = np.zeros(chunk_out2, dtype=np.float64)
adc_P0s_sum = np.zeros(chunk_out2, dtype=np.float64)
adc_L0f_sum = np.zeros(chunk_out2, dtype=np.float64)
adc_L0s_sum = np.zeros(chunk_out2, dtype=np.float64)

# Счётчики количества накоплений
adc_P0f_count = np.zeros(chunk_out2, dtype=np.int32)
adc_P0s_count = np.zeros(chunk_out2, dtype=np.int32)
adc_L0f_count = np.zeros(chunk_out2, dtype=np.int32)
adc_L0s_count = np.zeros(chunk_out2, dtype=np.int32)
...
def process_data(adc_P0, adc_L0, valid_starts, num_events):
    ...
    # Цикл для сбора значений фронтов и спадов
    for start_index in valid_starts:
        ...
        if front_start_P + chunk_out2 <= len(adc_P0):
            front_values_P[event_count, :] = adc_P0[front_start_P:front_start_P + chunk_out2]

        if spad_start_P + chunk_out2 <= len(adc_P0):
            spad_values_P[event_count, :] = adc_P0[spad_start_P:spad_start_P + chunk_out2]

        if front_start_L + chunk_out2 <= len(adc_L0):
            front_values_L[event_count, :] = adc_L0[front_start_L:front_start_L + chunk_out2]

        if spad_start_L + chunk_out2 <= len(adc_L0):
            spad_values_L[event_count, :] = adc_L0[spad_start_L:spad_start_L + chunk_out2]

        event_count += 1

    return front_values_P[:event_count], spad_values_P[:event_count], front_values_L[:event_count], spad_values_L[:event_count]


def callback(in_data, frame_count, time_info, status):
    global adc_L0, adc_P0, adc_P0f, adc_P0s, adc_L0f, adc_L0s
    global adc_P0f_sum, adc_P0s_sum, adc_L0f_sum, adc_L0s_sum
    global adc_P0f_count, adc_P0s_count, adc_L0f_count, adc_L0s_count
    global sinhro
    ...
    adc_L0[:len(data)] = data[:, 0] # Заполняем левый канал
    adc_P0[:len(data)] = data[:, 1] # Заполняем правый канал
    ...
    condition = (adc_P0[:-35] < threshold) & (adc_P0[1:-34] > threshold) & \
                ...
                (np.arange(len(adc_P0[:-35])) < 21500)  # Добавлено условие для индекса
...
# === НАКОПЛЕНИЕ ПОСТОЯННОЙ СОСТАВЛЯЮЩЕЙ ===
    adc_P0f_sum += adc_P0f
    adc_P0s_sum += adc_P0s
    adc_L0f_sum += adc_L0f
    adc_L0s_sum += adc_L0s
    ...
    # Проверяем, чтобы количество накоплений не превышало лимит
    adc_P0f_count = np.clip(adc_P0f_count, 1, accumulation_limit)
    adc_P0s_count = np.clip(adc_P0s_count, 1, accumulation_limit)
    adc_L0f_count = np.clip(adc_L0f_count, 1, accumulation_limit)
    adc_L0s_count = np.clip(adc_L0s_count, 1, accumulation_limit)

    # Вычисляем текущую усреднённую постоянную составляющую
    adc_P0f_mean = adc_P0f_sum / adc_P0f_count
    adc_P0s_mean = adc_P0s_sum / adc_P0s_count
    adc_L0f_mean = adc_L0f_sum / adc_L0f_count
    adc_L0s_mean = adc_L0s_sum / adc_L0s_count

    # === РЕАЛЬНАЯ КОМПЕНСАЦИЯ ПОСТОЯННОЙ СОСТАВЛЯЮЩЕЙ ===
    def compensate(value, mean):
        """
        Функция компенсирует значение `value` с учётом текущего усреднённого значения `mean`.
        Если значение больше mean, то вычитаем. Если меньше mean, то прибавляем.
        """
        return value - mean if value > mean else value + abs(mean)

    adc_P0f = np.vectorize(compensate)(adc_P0f, adc_P0f_mean).astype(np.int16)
    adc_P0s = np.vectorize(compensate)(adc_P0s, adc_P0s_mean).astype(np.int16)
    adc_L0f = np.vectorize(compensate)(adc_L0f, adc_L0f_mean).astype(np.int16)
    adc_L0s = np.vectorize(compensate)(adc_L0s, adc_L0s_mean).astype(np.int16)
