...
waveform_output_display = 5     # Разрешение вывода на экран: 0-без вывода; 1-сырого сигнала; 2-сигнала без постоянной; 3-постоянная составляющая сигнала
...
def callback(in_data, frame_count, time_info, status):
    ...
    if start_adr < 10:
        ...
        num_chunks = np.arange(12) * 1920
        #y_indices = np.arange(num_chunks)
        
           
        data_to_sum_p = np.sum([dataADC1[start_adress + 1920 * y:stop_adress + 1920 * y] for y in range(0,12)], axis=0)
        data_to_sum_n = np.sum([dataADC1[start_adress + 955 + 1920 * y:stop_adress + 955 + 1920 * y] for y in range(0,12)], axis=0)
    else:
        count_start = 0
...
#    p.terminate()
    ...
    while stream.is_active():
        ...
        if 10 > start_adr and count_start != 0 and count_sample != count_start and len(dataADC1) > CHUNK:                               # количество данных больше чем одно считывание
            ...
            dataDC = np.where(abs(dataINP11)<200, np.where(dataINP11>0, dataDC+tau1,dataDC-tau1), np.where(abs(dataINP11)>3000, np.where(dataINP11>0, dataDC+tau3, dataDC-tau3), np.where(dataINP11>0, dataDC+tau2,dataDC-tau2)))
            ...
            if waveform_output_display == 6:                         # отображаем результирующую осциллограмму усредненного сигнала 2-х циклов с компенсированным DC
                ...
                if temp < 10000:
                    ...
                    tau = 10
                dataINP_result1[mark12] = dataINP_result[mark12] - 500
                dataINP_result1[mark13] = dataINP_result[mark13] + 500
                dataINP_result1[mark14] = dataINP_result[mark14] - 500
                dataINP_result1[mark15] = dataINP_result[mark15] + 500
                dataINP_result1[mark16] = dataINP_result[mark16] - 500
                dataINP_result1[0] = dataINP_result[0] + 500
