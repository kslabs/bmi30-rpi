...
waveform_output_display = 0     # Разрешение вывода на экран: 0-без вывода; 1-сырого сигнала; 2-сигнала без постоянной; 3-постоянная составляющая сигнала
...
MAX_DATA_SIZE = CHUNK * 10  # ?????????? ????? ??? 10 ???????

dataADC = np.zeros(MAX_DATA_SIZE, dtype=np.int32)
dataADC1 = np.zeros(MAX_DATA_SIZE, dtype=np.int32)
dataADC2 = np.zeros(MAX_DATA_SIZE, dtype=np.int32)
dataADC3 = np.zeros(MAX_DATA_SIZE, dtype=np.int32)
dataADC4 = np.zeros(MAX_DATA_SIZE, dtype=np.int32)
dataADC5 = np.zeros(MAX_DATA_SIZE, dtype=np.int32)
dataADC6 = np.zeros(MAX_DATA_SIZE, dtype=np.int32)
dataADC7 = np.zeros(MAX_DATA_SIZE, dtype=np.int32)
dataINP1 = np.zeros(CHUNK_out2, dtype=np.int32)       # массив с компенсациЕЙ постоянной составляющей и АРУ
dataINP11 = np.zeros(CHUNK_out2, dtype=np.int32)       # массив с компенсациЕЙ постоянной составляющей 
dataINP_result = np.zeros(CHUNK_out2, dtype=np.int32)                   # Результирующий массив
dataINP_result1 = np.zeros(CHUNK_out2, dtype=np.int32)                   # Результирующий массив
dataINP_res_tau = np.zeros(CHUNK_out2, dtype=np.int32)              # массив длительностей роста и спада 

dataDC = np.zeros(CHUNK_out2, dtype=np.int32)         # массив с постоянной составляющей 
data_to_sum_p = np.zeros(CHUNK_out2, dtype=np.int32)
data_to_sum_n = np.zeros(CHUNK_out2, dtype=np.int32)
temp_add20 = np.zeros(20, dtype=np.int32)
flag_synchro = 0
ADCmax = 0

#@njit(fastmath=True)
def process_data(dataADC1, dataADC2, dataADC3, dataADC4, dataADC5, dataADC6, dataADC7, start_adr, count_start, start_adress, stop_adress, tau_start_adr, CHUNK, CHUNK0, flag_synchro, ADCmax):
    if len(dataADC1) > CHUNK:
        for i in range(CHUNK):
            if flag_synchro or (dataADC1[i] < 7000 and dataADC1[i + 1] > 7000 and dataADC1[i + 31] < -3000):
                ...
                start_adr = i
                dataADC7[:CHUNK] = dataADC6[:CHUNK]
                dataADC6[:CHUNK] = dataADC5[:CHUNK]
                dataADC5[:CHUNK] = dataADC4[:CHUNK]
                dataADC4[:CHUNK] = dataADC3[:CHUNK]
                dataADC3[:CHUNK] = dataADC2[:CHUNK]
                dataADC2[:CHUNK] = dataADC1[:CHUNK]
                print('shift_amount', CHUNK + (int(i / 1.2) if i > 16 else (1 if i > 8 else (0 if i > 3 else -1))))
                shift_amount = CHUNK + (int(i / 1.2) if i > 16 else (1 if i > 8 else (0 if i > 3 else -1)))
                

                # ?????? np.roll
                shifted_data = np.empty_like(dataADC1)
                shifted_data[-shift_amount:] = dataADC1[:shift_amount]
                shifted_data[:-shift_amount] = dataADC1[shift_amount:]

                print(len(dataADC1), len(shifted_data))
                dataADC1[:] = shifted_data[:]
                break
            if i > CHUNK0:
                dataADC7[:CHUNK] = dataADC6[:CHUNK]
                dataADC6[:CHUNK] = dataADC5[:CHUNK]
    ...
    if start_adr < 10 and tau_start_adr == 0:
        ...
        dataADC_size = CHUNK * 7  
        dataADC = np.empty(dataADC_size, dtype=dataADC1.dtype)  
        dataADC[:CHUNK] = dataADC7[:CHUNK]
        dataADC[CHUNK:2 * CHUNK] = dataADC6[:CHUNK]
        dataADC[2 * CHUNK:3 * CHUNK] = dataADC5[:CHUNK]
        dataADC[3 * CHUNK:4 * CHUNK] = dataADC4[:CHUNK]
        dataADC[4 * CHUNK:5 * CHUNK] = dataADC3[:CHUNK]
        dataADC[5 * CHUNK:6 * CHUNK] = dataADC2[:CHUNK]
        dataADC[6 * CHUNK:] = dataADC1
        
        data_to_sum_p = np.zeros(stop_adress - start_adress, dtype=np.int64)
        data_to_sum_n = np.zeros(stop_adress - start_adress, dtype=np.int64)
        segment_len = stop_adress - start_adress
        num_segments = len(dataADC) // 1920
        for segment_index in range(num_segments):
            p_start = start_adress + segment_index * 1920
            n_start = p_start + 955
            for k in range(segment_len):
                data_to_sum_p[k] += dataADC[p_start + k]
                data_to_sum_n[k] += dataADC[n_start + k]

        data_to_sum_p //= num_segments
        data_to_sum_n //= num_segments

        count_start += 1
    else:
        count_start = 0

    return dataADC1, dataADC2, dataADC3, dataADC4, dataADC5, dataADC6, dataADC7, count_start, data_to_sum_p, data_to_sum_n, flag_synchro, ADCmax
...
def callback(in_data, frame_count, time_info, status):
    ...
    new_data = np.frombuffer(in_data, dtype=np.int32)
    
    global dataADC1, dataADC2, dataADC3, dataADC4, dataADC5, dataADC6, dataADC7, count_start, data_to_sum_p, data_to_sum_n, start_adr, start_adress, stop_adress, tau_start_adr, flag_synchro, ADCmax
    print(len(dataADC1), len(dataADC2), len(dataADC3), len(dataADC4), len(dataADC5), type(count_start), len(data_to_sum_p), len(data_to_sum_n), type(flag_synchro), type(ADCmax))
    print(type(dataADC1), type(dataADC2), type(dataADC3), type(dataADC4), type(dataADC5), type(new_data), type(start_adr), type(count_start), type(data_to_sum_p), type(data_to_sum_n), type(flag_synchro), type(ADCmax))

    dataADC1, dataADC2, dataADC3, dataADC4, dataADC5, dataADC6, dataADC7, count_start, data_to_sum_p, data_to_sum_n, flag_synchro, ADCmax = process_data(dataADC1, dataADC2, dataADC3, dataADC4, dataADC5, dataADC6, dataADC7, start_adr, count_start, start_adress, stop_adress, tau_start_adr, CHUNK, CHUNK0, flag_synchro, ADCmax)
...
if 1==1:
    ...
    while stream.is_active():
        ...
        if start_adr > 9:
            ...
            #print('--------', len(dataINP_result1), len(dataINP_result))
            ...
            dataINP_count = np.zeros(CHUNK_out2, dtype=np.int32)
            ...
            flag_znak= 0
            dataINP_res_tau = np.zeros(CHUNK_out2, dtype=np.int32)
            count_y = 0
            ...
            if count_time_detect > 10 and flag_first_alignment: 
                ...
                if flag_det_tag > 0 and abs(mean_dataINP_count) < 15 and diff_sum_first > 150 and diff3[0]>0 and max_mean <20 and flag_diff4 ==0:   # первые 10 сек не детектировать и проверять условие: средняя составляющая должна быть меньше 33. 
                    # Метка найдена

                        
                    print(marked_tau, f'{count_delay_tag:4}', f'Метка определена с макс. уровнем сигнала:{dataINP_max:6}', f'| Mean:{mean_dataINP_count:4}', f'/{max_mean:4}',f'/{diff_sum_first:4}','/',flag_diff4, diff3[22:36], '      !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!       ', end='\r')
                    ...
                    if count_delay_tag >0:
                        ...
                        if abs(mean_dataINP_count) > max_mean:
                            max_mean = abs(mean_dataINP_count)
                    print(marked_tau, f'{count_delay_notag:4}', f'Метки нет.         Макс. уровень сигнала:{dataINP_max:6}', f'| Mean:{mean_dataINP_count:4}', f'/{max_mean:4}',f'/{diff_sum_first:4}','/',flag_diff4,diff3[22:36], '      --------------------------------------      ', end='\r')
                    count_delay_tag = 0
                    count_delay_notag += 1
            ...
            if waveform_output_display == 1:                         # отображаем осциллограмму сырого входного сигнала
                ...
                #dataADC1[start_adr+21120] = dataADC1[start_adr+21120] + 1000
