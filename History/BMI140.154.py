...
waveform_output_display = 10     # Разрешение вывода на экран: 0-без вывода; 1-сырого сигнала; 2-сигнала без постоянной; 3-постоянная составляющая сигнала
...
def callback(in_data, frame_count, time_info, status):
    ...
    if len(dataADC1) > CHUNK:
        ...
        for i in range(0, CHUNK):
            if flag_synchro or (dataADC1[i] < 5000 and dataADC1[i+1] > 5000 and dataADC1[i+24] > 4000):
                flag_synchro = 1
    ...
    #dataADC1 = np.concatenate((dataADC1, np.frombuffer(in_data[0::2], dtype=np.int16)))
    #dataADC01 = np.concatenate((dataADC01, np.frombuffer(in_data[1::2], dtype=np.int16)))
