...
def callback(in_data, frame_count, time_info, status):
    ...
    global start_adr, count_start, data_to_sum_p, data_to_sum_n, start_adress, stop_adress, tau_start_adr, segment_length
    flag_synchro = 0
    ADCmax = 0
    ...
    if start_adr < 10 and tau_start_adr == 0:
        ...
        y_indices_p = np.arange(num_segments) * 1920 + start_adress
        y_indices_n = np.arange(num_segments) * 1920 + start_adress + 960

        data_to_sum_p = np.mean(dataADC[y_indices_p].reshape(num_segments, segment_length), axis=1)
        data_to_sum_n = np.mean(dataADC[y_indices_n].reshape(num_segments, segment_length), axis=1)
    else:
        count_start = 0
