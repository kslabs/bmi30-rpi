...
def update_graph(show_plot, update_needed, current_oscilloscope, start_sample, num_samples, 
    ...
    if show_plot:
        ...
        if current_oscilloscope == 1:
            ...
            # Скрываем основные линии (но не удаляем их)
            line1.set_visible(False)
            line2.set_visible(False)
            
            # Очищаем дополнительные линии, если они есть
            for i in range(len(ax.lines)-2):  # Оставляем только две первые линии
                if i + 2 < len(ax.lines):
                    ax.lines[i+2].remove()
            
            # Добавляем периоды с защитой от ошибок
            try:
                period = 1920
                ...
                for i in range(12):
                    ...
                    if start_idx + 200 <= len(dataADC_P1):
                        data = dataADC_P1[start_idx:start_idx + 200].copy()  # Создаем копию данных
                        if len(data) == 200:  # Проверяем размер данных
                            # Центрируем данные только если dc_P1_1 не пустой
                            if len(dc_P1_1) == 200:
                                data = data - np.mean(dc_P1_1)
                            ax.plot(range(200), data, 
                                    color=colors[i], 
                                    alpha=0.7,
                                    label=f'Период {i+1}')
                            added_lines += 1
                ...
                print(f"Ошибка при отображении периодов: {e}")
                # Восстанавливаем стандартные линии в случае ошибки
                line1.set_visible(True)
                line2.set_visible(True)
            ...
            return True, False  # Возвращаемся без дополнительной обработки
