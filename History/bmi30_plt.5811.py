...
def update_graph(show_plot, update_needed, current_oscilloscope, start_sample, num_samples, 
                ...
                correlation_dc_component=None):  # Добавляем новый параметр
    ...
    if show_plot:
        ...
        if current_oscilloscope == 1:
        ...
        elif current_oscilloscope == 28:
            # Отображаем DC-компоненту корреляции, если она есть
            ax.clear()  # Сначала очищаем график
            
            # Создаем список линий и меток для легенды
            legend_lines = []
            legend_labels = []
            
            # Проверяем доступность данных DC-компоненты
            if 'correlation_dc_component' in globals() and correlation_dc_component is not None and len(correlation_dc_component) > 0:
                shifts = np.arange(-100, 101)
                
                # Рисуем DC-компоненту
                dc_line, = ax.plot(shifts, correlation_dc_component, 'r-', linewidth=2)
                legend_lines.append(dc_line)
                legend_labels.append('DC корреляции')
                
                # Если есть текущая разница корреляций, добавляем для сравнения
                if cross_correlation_sum is not None and antiphase_correlation_sum is not None:
                    correlation_diff = cross_correlation_sum - antiphase_correlation_sum
                    diff_line, = ax.plot(shifts, correlation_diff, 'b-', linewidth=1, alpha=0.6)
                    legend_lines.append(diff_line)
                    legend_labels.append('Текущая разница')
                    
                    # Добавляем AC-компоненту тоже
                    if len(correlation_dc_component) == len(correlation_diff):
                        correlation_ac = correlation_diff - correlation_dc_component
                        ac_line, = ax.plot(shifts, correlation_ac, 'g-', linewidth=1)
                        legend_lines.append(ac_line)
                        legend_labels.append('AC компонента')
                
                # Настраиваем график
                ax.set_xlim(-100, 100)
                ax.set_title('DC-компонента графика разности корреляций')
                ax.set_xlabel('Смещение (семплы)')
                ax.set_ylabel('Амплитуда')
                ax.grid(True, linestyle=':', alpha=0.7)
                
                # Обновляем основные линии
                line1.set_data(shifts, correlation_dc_component)
                line2.set_data([], [])
            else:
                # Если DC-компоненты нет - настраиваем пустой график
                ax.set_title('DC-компонента графика разности корреляций (нет данных)')
                ax.set_xlabel('Смещение (семплы)')
                ax.set_ylabel('Амплитуда')
                line1.set_data([], [])
                line2.set_data([], [])
            
            # Создаем легенду только если есть линии с метками
            if legend_lines and legend_labels:
            ...
            if cross_correlation_sum is not None and antiphase_correlation_sum is not None and len(cross_correlation_sum) > 0 and len(antiphase_correlation_sum) > 0:
                ...
                # Получаем DC и AC компоненты корреляции, если они доступны в глобальных переменных
                dc_comp = None
                ac_comp = None
                
                # Импорт должен быть внутри условия, чтобы избежать циклических импортов
                import sys
                if 'correlation_dc_component' in sys.modules['__main__'].__dict__:
                    dc_comp = sys.modules['__main__'].correlation_dc_component
                    if dc_comp is not None and len(dc_comp) == len(correlation_diff):
                        ac_comp = correlation_diff - dc_comp
                ...
                legend_handles = [diff_line]
                legend_labels = ['Разница корреляций']
                
                # Добавляем DC и AC компоненты, если они доступны
                if dc_comp is not None and len(dc_comp) == len(correlation_diff):
                    dc_line, = ax.plot(shifts, dc_comp, 'r-', linewidth=1, alpha=0.7, label='DC компонент')
                    legend_handles.append(dc_line)
                    legend_labels.append('DC компонент')
                    
                    if ac_comp is not None:
                        ac_line, = ax.plot(shifts, ac_comp, 'g-', linewidth=1, alpha=0.7, label='AC компонент')
                        legend_handles.append(ac_line)
                        legend_labels.append('AC компонент')
                
                # Добавляем легенду явно с указанными объектами и метками
                try:
                    if len(legend_handles) > 0 and len(legend_labels) > 0:
                        ax.legend(legend_handles, legend_labels, loc='upper right', fontsize=8)
                except Exception as e:
                    # Игнорируем ошибки с легендой
                    pass
                
                # Устанавливаем заголовок и оси
                ax.set_title('Разница корреляций', fontsize=10)
                ax.set_xlabel('Смещение')
                ax.set_ylabel('Амплитуда')
                
                # Включаем сетку
                ax.grid(True, linestyle=':', alpha=0.7)
                
                # Выводим информацию о SNR и обнаружении, если есть доступ к глобальным переменным
                try:
                    if 'snr' in sys.modules['__main__'].__dict__ and 'correlation_sound_active' in sys.modules['__main__'].__dict__:
                        snr = sys.modules['__main__'].snr
                        threshold = sys.modules['__main__'].adaptive_correlation_threshold
                        sound_active = sys.modules['__main__'].correlation_sound_active
                        
                        # Добавляем информацию о SNR
                        ax.text(0.02, 0.75, f'SNR: {snr:.2f} / {threshold*1.1:.2f}', 
                                transform=ax.transAxes, fontsize=9,
                                bbox=dict(boxstyle='round', facecolor='lightgreen' if snr > 1.0 else 'white', alpha=0.6))
                        
                        # Добавляем информацию о состоянии звука
                        if sound_active:
                            ax.text(0.02, 0.65, f'BEEP: ON', transform=ax.transAxes, fontsize=9,
                                   bbox=dict(boxstyle='round', facecolor='lightcoral', alpha=0.6))
                except:
                    pass
                
                # Обновляем линии для последующих обновлений
                line1.set_data(shifts, correlation_diff)
                line2.set_data([], [])  # Вторая линия не используется

                # Если доступен градиентный индекс и порог, добавляем их на график
                import sys
                distortion_index = None
                gradient_threshold = None

                if '__main__' in sys.modules:
                    if hasattr(sys.modules['__main__'], 'distortion_index'):
                        distortion_index = sys.modules['__main__'].distortion_index
                    if hasattr(sys.modules['__main__'], 'gradient_threshold'):
                        gradient_threshold = sys.modules['__main__'].gradient_threshold

                if distortion_index is not None and gradient_threshold is not None:
                    # Добавляем информацию о градиентном методе
                    ax.text(0.02, 0.85, f'Индекс искажения: {distortion_index:.0f}', 
                            transform=ax.transAxes, fontsize=9,
                            bbox=dict(boxstyle='round', facecolor='lightyellow', alpha=0.6))

                    ax.text(0.02, 0.80, f'Порог: {gradient_threshold:.0f}', 
                            transform=ax.transAxes, fontsize=9,
                            bbox=dict(boxstyle='round', facecolor='lightgreen' if distortion_index > gradient_threshold else 'white', alpha=0.6))
            else:
        ...
        try:
            ...
            if handles and labels:  # Проверяем, что есть объекты для легенды
                ax.legend(handles, labels)
        except:
            pass  # Игнорируем ошибки с легендой
...
def compute_adaptive_correlation_threshold(correlation_diff, correlation_index=None, 
                                         sound_active=False, base_threshold_ratio=0.2,  # Увеличен до 20%
                                         current_threshold=None, adaptation_rate=0.05):
    """
    Анализирует форму графика разности корреляций для обнаружения сигнала
    с оценкой шума на основе AC-компоненты
    
    Args:
        correlation_diff: Массив AC-компоненты разности корреляций
        correlation_index: Не используется (оставлен для совместимости)
        sound_active: Активен ли звуковой сигнал в данный момент
        base_threshold_ratio: Процент превышения над шумом (0.2 = 20%)
        current_threshold: Текущее значение SNR порога
        adaptation_rate: Скорость адаптации порога
    
    Returns:
        tuple: (текущий SNR, флаг обнаружения сигнала)
    """
    ...
    # Вычисляем статистики сигнала
    signal_values = [correlation_diff[i] for i in signal_area]
    signal_mean = np.mean(signal_values)
    signal_max = max([abs(val - noise_mean) for val in signal_values])
    
    # Определяем пороговое значение - 20% выше уровня шума
    noise_threshold = noise_std * (1 + base_threshold_ratio)  # Базовый порог +20% от уровня шума
    
    # Вычисляем SNR как отношение максимального отклонения сигнала к порогу шума
    snr = signal_max / max(1, noise_threshold)
    
    # Порог SNR для обнаружения сигнала - 1.0 означает "точно на пороге"
    snr_threshold = 1.0
    
    # Определяем обнаружение - SNR должно превышать 1.0
    # (то есть сигнал должен быть выше шума+20%)
...
def compute_correlation_dc(correlation_diff, alpha=0.005):
    ...
    if correlation_dc_component is None:
        print("DC-компонента корреляции инициализируется средним значением сигнала")
        ...
        print(f"Размеры не совпадают: DC={len(correlation_dc_component)}, сигнал={len(correlation_diff)}")
        correlation_dc_component = np.full_like(correlation_diff, np.mean(correlation_diff))
