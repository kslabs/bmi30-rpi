...
version = 35.00  # Обновляем версию
...
# Функция для реализации прерывистого звукового сигнала
def toggle_beep():
    global beep_state
    if not beep_mode == "intermittent":
        return  # Если не в режиме прерывистого звука, не делаем ничего
        
    beep_state = not beep_state  # Инвертируем состояние
    if beep_state:
        gpio13.on()  # Включаем звук
    else:
        gpio13.off()  # Выключаем звук
        
    # Перезапускаем таймер, если всё ещё в режиме прерывистого сигнала
    if beep_mode == "intermittent":
        global beep_timer
        beep_timer = threading.Timer(0.05, toggle_beep)  # 0.1 с = частота 5 Гц (вкл-выкл)
        beep_timer.daemon = True  # Таймер завершится при завершении программы
        beep_timer.start()

# Функция для запуска прерывистого сигнала
def start_intermittent_beep():
    global beep_timer, beep_state
    # Остановим существующий таймер, если он есть
    if beep_timer:
        beep_timer.cancel()
    ...
    # Запускаем таймер для переключения
    beep_timer = threading.Timer(0.05, toggle_beep)  # 0.1 с = частота 5 Гц
    beep_timer.daemon = True
    beep_timer.start()

# Функция для остановки прерывистого сигнала
def stop_beep():
    global beep_timer
    if beep_timer:
        beep_timer.cancel()
        beep_timer = None
    gpio13.off()  # Гарантированно выключаем звук
...
try:
    ...
    while True:
        ...
        debug_output = 1  # Установите True/False для вывода отладочной информации
        if debug_output:
            print(f"Амплитуда: {max_amp_15:.0f}, Фаза: {max_phase_15:.0f}, Порог: {amplitude_threshold}")
            print(f"Изменение фазы: {abs(max_phase_15 - prev_max_phase_15):.0f}, Порог стабильности: {stable_phase_threshold}")
            print(f"Последовательных обнаружений: {consecutive_detections}")
        
        # Условие обнаружения сигнала
        if max_amp_15 > amplitude_threshold:
            ...
            if abs(max_phase_15 - prev_max_phase_15) <= stable_phase_threshold:
                ...
                # Сигнализируем только если максимум стабилен в течение как минимум 2 отсчетов
                if consecutive_detections > 2:
                    # Принудительно включаем BEEP в зависимости от выбранного режима
                    if beep_mode == "continuous":
                        # Постоянный сигнал
                        stop_beep()  # Останавливаем таймер, если он был
                        gpio13.on()  # Просто включаем постоянный звук
                    else:
                        # Прерывистый сигнал
                        start_intermittent_beep()  # Запускаем прерывистый сигнал
                    ...
                    print(f"Сигнал обнаружен! Амплитуда: {max_amp_15:.0f}, Фаза: {max_phase_15}, Δфазы: {last_phase_diff:.0f}")
            ...
            if debug_output:
                ...
                if max_amp_15 > 0 and max_amp_15 < amplitude_threshold:
                    print(f"Текущая амплитуда ниже порога: {max_amp_15:.0f} < {amplitude_threshold}")
            ...
            for period in silent_warning_periods:
                if silent_time >= period and not silent_warnings_shown[period]:
                    ...
                    print(f"⚠️ ВНИМАНИЕ! Прошло {period_min}м {period_sec}с без обнаружения сигнала!")
