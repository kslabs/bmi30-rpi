...
class ScopeWindow:
	...
	def _tick(self):
		...
		try:
			...
			while True:
				...
				# Фильтр полностью нулевых блоков: иногда прошивка/транспорт отдаёт "пустые" кадры,
				# из-за чего на графике всплесками видны одни нули. По умолчанию такие кадры скрываем.
				try:
					if self.hide_all_zero and len(ch0) and len(ch1) and np.all(ch0 == 0) and np.all(ch1 == 0):
						self.zero_blocks += 1
						print("[zero] Полностью нулевой стерео-блок — пропущен")
						# не обновляем data0/data1, оставляем последний валидный кадр на экране
						continue
				except Exception:
					pass
	...
	def _on_freq_change(self, idx:int):
		...
		if self.stream is None:
			...
			return
		# Переключение на лету: отправим профиль и соответствующий NS и старт
		try:
			...
			ns = 912 if self.desired_profile == 2 else 1360
			self.stream.send_cmd(CMD_SET_FRAME_SAMPLES, int(ns).to_bytes(2,'little'))
			try:
				import time as _t
				_t.sleep(0.02)
