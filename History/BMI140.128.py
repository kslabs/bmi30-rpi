...
waveform_output_display = 2     # Разрешение вывода на экран: 0-без вывода; 1-сырого сигнала; 2-сигнала без постоянной; 3-постоянная составляющая сигнала
                                # 
...
def callback(in_data, frame_count, time_info, status):
    ...
    global start_adr
    global count_start
    global data_to_sum_p
    global data_to_sum_n
    global start_adress
    global stop_adress
    flag_synchro = 0
    ADCmax = 0
        ...
        for i in range(0, CHUNK):
            if flag_synchro or (dataADC1[i] < 7000 and dataADC1[i+1] > 7000 and dataADC1[i+31] < -3000):
                ...
                if dataADC1[i] > ADCmax:
                    ...
                    dataADC2 = np.copy(dataADC1)
                    if i > 16:
                        #dataADC2 = np.copy(dataADC1[:CHUNK2 + int(i / 1.2)])
                        dataADC1 = dataADC1[CHUNK + int(i / 1.2):]
                    else:
                        if i > 8:
                            #dataADC2 = np.copy(dataADC1[:CHUNK2 + 1])
                            dataADC1 = dataADC1[CHUNK + 1:]
                        else:
                            if i > 3:
                                #dataADC2 = np.copy(dataADC1[:CHUNK2])
...
#    p.terminate()
    ...
    while stream.is_active():
        ...
        if 10 > start_adr and count_start != 0 and count_sample != count_start and len(dataADC1) > CHUNK:                               # количество данных больше чем одно считывание
        #if 1==1:
            ...
            #data_to_sum_p = np.sum([dataADC1[start_adress + (1920 * y) : stop_adress + (1920 * y)] for y in range(int(len(dataADC1)/1920))], axis=0)
            #data_to_sum_n = np.sum([dataADC1[start_adress + 955 + (1920 * y) : stop_adress + 955 + (1920 * y)] for y in range(int(len(dataADC1)/1920))], axis=0)

            # Генерируем все индексы для суммирования
            #y_indices = np.arange(int(len(dataADC1)/1920)-1) * 1920
            ...
            #print(len(indices_p), len(indices_n), indices_p)
            
            # суммирование значений по индексам
            ...
            if 1==0 and dataINP_max > trigger_treshold + (int(count_delay_tag / 100) * 10000):
                flag_det_tag += 1
                marked_tau = '+'
            else:
                if count_delay_notag > 100:
                    dataDC = np.where(abs(dataINP11)<10000, np.where(dataINP11>0, dataDC+tau1,dataDC-tau1), np.where(abs(dataINP11)>trigger_treshold, np.where(dataINP11>0, dataDC+tau3, dataDC-tau3), np.where(dataINP11>0, dataDC+tau2,dataDC-tau2)))
                
                if dataINP_max < trigger_treshold:
                    flag_det_tag = 0
                    marked_tau = '-'
                else:
                    marked_tau = '~'
            ...
            temp2 = np.diff(dataINP_res_tau, n=1, prepend=0)
            temp3 = np.diff(dataINP_res_tau, n=2, prepend=0, append=0)
            temp4 = np.diff(temp3, n=2, prepend=0, append=0)
            temp5 = np.diff(temp3, n=2, prepend=0, append=0)
            temp6 = np.diff(temp5, n=2, prepend=0, append=0)
            temp7 = np.diff(temp5, n=2, prepend=0, append=0)
            temp8 = np.diff(temp7, n=2, prepend=0, append=0)
            temp9 = np.diff(temp7, n=2, prepend=0, append=0)
            temp10 = np.diff(temp9, n=2, prepend=0, append=0)
            temp11 = np.diff(temp9, n=2, prepend=0, append=0)
            temp12 = np.diff(temp11, n=2, prepend=0, append=0)
            temp13 = np.diff(temp11, n=2, prepend=0, append=0)
            temp14 = np.diff(temp13, n=2, prepend=0, append=0)
            temp15 = np.diff(temp13, n=2, prepend=0, append=0)
            temp16 = np.diff(temp15, n=2, prepend=0, append=0)
            temp17 = np.diff(temp15, n=2, prepend=0, append=0)
            temp18 = np.diff(temp17, n=2, prepend=0, append=0)
            temp19 = np.diff(temp17, n=2, prepend=0, append=0)
            temp20 = np.diff(temp19, n=2, prepend=0, append=0)
            temp21 = np.diff(temp19, n=2, prepend=0, append=0)/10
            #print('dataINP_res_tau', dataINP_res_tau[0:36])
            #print('TEMP2', temp2[0:80])
            #print('TEMP3', temp3[0:80])
            #print('TEMP20', temp18[0:80])
            #print('TEMP21', temp19[0:80])
            #print(np.count_nonzero(temp21), np.sum(np.abs(temp21[0:7])) / np.sum(np.abs(temp21[7:14]))  )                  # количество значений отличающихся от нуля
            #print('dataINP_count:', dataINP_count)
            #exp_dataINP_count = np.exp(dataINP_result)

            #print('dataINP_result:', np.mean(dataINP_result), dataINP_result)
            #print('dataINP_count', np.mean(dataINP_count), dataINP_count)
            #print(np.mean(dataINP_count))

            #diff = np.zeros(CHUNK_out2, 'int64')
            #diff1 = np.diff(dataINP_result, n=1, prepend=0)                              #, append=0)
            
            #diff = np.diff(diff) / 2
            
            #print(len(diff), len(diff1), len(diff2), len(dataINP_result))
            '''
            
            for i in range(1, CHUNK_out2):
                if diff1[i] > diff1[i-1]:
                    if flag_znak:
                        dataINP_count[i] = dataINP_count[i-1] + 1
                        if dataINP_count[i] > 20:
                            print('+', i, ':', dataINP_count[i]) 
                    else:
                        dataINP_count[i] = int(dataINP_count[i-1] / 2)
                        flag_znak = 1
                else:
                    if diff1[i] < diff1[i-1]:
                        if flag_znak:
                        ...
                        dataINP_count[i] = dataINP_count[i-1]
            ...
            #
            dataINP_count[2:] = np.where(increasing[1:], 
                              np.where(flag_znak, 
                                       dataINP_count[1:-1] + 1, 
                                       dataINP_count[1:-1] // 2),
                              np.where(decreasing[1:], 
                                       np.where(flag_znak, 
                                                dataINP_count[1:-1] - 1, 
                                                dataINP_count[1:-1] // 2),
                                       dataINP_count[1:-1]))
            ...
            #
            flag_znak = np.where(increasing[1:], 1, np.where(decreasing[1:], 0, flag_znak))
            '''
            #diff = np.append(diff, 0)
            #diff = np.append(diff, 0)
            #diff = np.append(diff, 0)
            #differences = np.diff(dataINP_result, prepend=dataINP_result[0])
            #dataINP_count = np.cumsum(np.sign(differences))

            #comparison = dataINP_result[:-1] > dataINP_result[1:]
            #dataINP_count[:-1] += np.where(comparison, 1, -1) 
            
            #print(len(dataINP_result[:-1]), len(dataINP_result[1:]))

            
            #comparison = dataINP_result[:] > 0
            #dataINP_count = np.where(dataINP_result[1:-1]>dataINP_result[1:-1], np.where(dataINP_result[1:]>0, dataINP_count[:-1] + 1, 0), np.where(dataINP_result[:-1]<0, np.where(dataINP_result[1:]<0, dataINP_count[:-1] - 1, 0),0)) 
            ...
            if count_time_detect > 10: 
                if flag_det_tag > 0 and abs(mean_dataINP_count) < 20:   # первые 10 сек не детектировать и проверять условие: средняя составляющая должна быть меньше 33. 
                    # Метка найдена
                    ...
                    print(marked_tau, f'{count_delay_tag:4}', f'Метка определена с максимальным уровнем сигнала:{dataINP_max:6}', f'| Mean:{mean_dataINP_count:4}', f'/{max_mean:4}', '      !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!       ', end='\r')
                    ...
                    if count_delay_tag >0:
                        ...
                        if abs(mean_dataINP_count) > abs(max_mean):
                            max_mean = mean_dataINP_count
                    print(marked_tau, f'{count_delay_notag:4}', f'Метки нет.         Максимальный уровень сигнала:{dataINP_max:6}', f'| Mean:{mean_dataINP_count:4}', f'/{max_mean:4}', '      ----------------------------------------------------------      ', end='\r')
                    count_delay_tag = 0
                    count_delay_notag += 1


            else:
                if int(time.time() - timer_test) != count_time_detect:
                    print('Задержка включения детекции метки на:', count_time_detect, '/10 сек', f'| Mean:{mean_dataINP_count:3}', '      ', end='\r')
                    count_time_detect = int(time.time() - timer_test)

            #dataDC = np.where(abs(dataINP11)<1000, np.where(dataINP11>0, dataDC+int(dataINP11/10), dataDC-int(dataINP11/10)), np.where(abs(dataINP11)>10000, np.where(dataINP11>0, dataDC+int(dataINP11/1000), dataDC-int(dataINP11/1000)), np.where(dataINP11>0, dataDC+int(dataINP11/100), dataDC-int(dataINP11/100))))
            ...
            if waveform_output_display == 5:                         # отображаем осциллограмму DC
                ...
                fig.canvas.draw()
                fig.canvas.flush_events()
            if waveform_output_display == 6:                         # отображаем результирующую осциллограмму усредненного сигнала 2-х циклов с компенсированным DC
            ...
            if waveform_output_display == 8:                         # отображаем осциллограмму DC
                display_line.set_ydata(temp21/30000)
                fig.canvas.draw()
                fig.canvas.flush_events()
