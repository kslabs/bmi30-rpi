...
# Основной цикл ожидания прерываний (без нагрузки на CPU)
try:
    while True:
        event20 = line20.event_read()  # Ждём передний фронт на INPUT_GPIO1
        if event20:
            change_pwm1()

        event21 = line21.event_read()  # Ждём передний фронт на INPUT_GPIO2
        if event21:
            change_pwm2()
