...
def run(profile:int, ns:int|None, rate:int|None, duration:float):
    ...
    try:
        ...
        pass
    time.sleep(0.05)
    send_cmd(dev, CMD_SET_PROFILE, bytes([profile & 0xFF])); time.sleep(0.15)
    # Не отправляем SET_FRAME_SAMPLES для profile=1 (ломает), отправляем для profile=2 или если явно указано
    if profile == 2 or (ns is not None and profile == 2):
        ns_val = int(ns) if ns is not None else 912
        send_cmd(dev, CMD_SET_FRAME_SAMPLES, ns_val.to_bytes(2,'little')); time.sleep(0.02)
    
    # Отправляем SET_BLOCK_RATE только если явно передано
    if rate is not None:
        try:
            send_set_block_rate(dev, int(rate)); time.sleep(0.02)
        except Exception:
            pass
    
    time.sleep(0.05)
    send_cmd(dev, CMD_START_STREAM, b""); time.sleep(0.15)
    wait_ready(dev, timeout=0.6)
