...
class ScopeWindow:
	def __init__(self):
		...
		self.expected_len_map = {1:1200, 2:1200}
		self.initial_expected = self.expected_len_map.get(1, 1200)
		...
		self.max_samples = 1200  # максимум: устройство теперь шлёт 1200 samples
		self.data0 = np.zeros(self.max_samples, dtype=np.int32)  # храним как unsigned -> int32 для удобства
		...
		self._instr = ("Инструкция: 1) Прошивка должна обрабатывать START_STREAM (0x20) и слать кадры vendor bulk на EP IN 0x83. "
			...
			"Каждый кадр: заголовок 32 байта (magic 0xA55A LE), флаги 0x01 (ADC0) и 0x02 (ADC1) чередуются; total_samples=1200, payload = samples*2 байт. "
					...
					if getattr(self, 'debug_markers', False):
						try:
							...
							if buf_len > 0:
								...
								self.data0[:buf_len] = vals
								self.data1[:buf_len] = vals
								if buf_len < self.max_samples:
									self.data0[buf_len:] = 0
									self.data1[buf_len:] = 0
								try:
	...
	def _try_connect(self, first=False):
		...
		try:
			...
			try:
				...
				pass
			# Устройство теперь шлёт 1200 samples в обоих профилях
			fs = 1200 if self.send_ns else None
	...
	def _on_freq_change(self, idx:int):
		...
		try:
			...
			try:
				...
				CMD_SET_FRAME_SAMPLES = 0x17
			# В рабочем режиме устройство шлёт 1200 samples для обоих профилей
			ns = 1200
			self.stream.send_cmd(CMD_SET_FRAME_SAMPLES, int(ns).to_bytes(2,'little'))
			try:
				import time as _t
