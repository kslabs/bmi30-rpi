...
def main():
    d,intf=open_dev()
    try:
        # профиль 300 Гц (912), полный режим, старт
        try:
            w(d, bytes([CMD_SET_PROFILE, 0x02])); time.sleep(0.03)
            w(d, bytes([CMD_SET_FULL, 0x01])); time.sleep(0.03)
        except Exception:
            pass
        w(d, bytes([CMD_START])); time.sleep(0.06)
        buf=bytearray(); MAGIC_LE=b"\x5A\xA5"; frames=0
        ...
        while time.time()-t0<3.0 and frames<3:
            ...
            while True:
                ...
                # пропустим тестовый кадр 8 семплов
                ...
                if not first_print:
                    print(f"FRAME seq={seq} total={total_samples} (ожидаем 912 для 300Гц)")
                    first_print=True
