...
- 0x13, 0x14, 0x18, 0x19, 0x1D, 0x33, 0x34, 0x3B, 0x3C, 0x3D, 0x41, 0x44 (u8 в младшем байте wValue; для 0x1D режимы MASTER/SLAVE запрещены без timestamp)
- 0x17 (u16 в wValue)
...
- `optic_active` не фильтруется по `sync_ok_visual`, `sync_locked`, роли или
  качеству связи. Постоянный физический `PD0=1` всегда остаётся `1` для RSP;
  SYNC-состояние отображается отдельно и не должно мигать оптическим индикатором.
- Если удалённый ID отсутствует в свежем `sync_seen_mask`, RSP не подменяет его
  состояние нулём, а сохраняет последнее принятое значение. Менять optic-
  состояние можно только по новому status byte присутствующего ID.
...
Оптическая карта и назначение источника системного LED:
...
- Master optic уже передается по RS485 в `master_status0 bit5`; firmware на slave
  добавляет его в общую таблицу по постоянному `master_device_id`. Legacy-поле
  `flags_runtime & 0x0080` остаётся только для совместимости и не выбирает источник LED.
- Onboard/system address WS2812 при `sync_signal_alive=0` показывает единый голубой
  ...
  Оптический цвет появляется только для ID, который RSP передал командой `0x44`:
  собственный ID — жёлтый на slave или зелёный на master; любой выбранный соседний
  ID — магента/фиолетовый. `0x44,0xFF` отключает реакцию.
- `optic_active=1` выбранного источника имеет на системном WS2812 высший приоритет
  и немедленно меняет только цвет. Индикация включенного TX остаётся независимой:
  выбранный цвет продолжает плавно мигать (`TX-breathe`) весь интервал.
- LCD-поле `OPT:0/1` показывает тот же удержанный `optic_active`; сырой `PD0` оставлен
  только в расширенной COM-диагностике `OPTIC`.
- RSP должен читать выбранный источник из `sync_status_bytes[source_id] bit5` или
  `EVT1 SENSOR_MAP`; использовать master-флаг для выбора реакции запрещено.
...
#### Обязательная дедупликация на host по STM32 UID96

Полный `STM32 UID96` является постоянным уникальным ключом физической платы. `device_id`, роль,
номер RPI и IP являются изменяемыми атрибутами этой записи и не могут использоваться как primary
key устройства. Требования к backend и frontend портала:

- нормализовать UID96 в один формат (24 hex-символа) и хранить не более одной текущей записи на UID;
- при получении уже известного UID с новым `device_id` атомарно перенести существующую запись на
  новый ID, удалить старую привязку `device_id -> UID` и не создавать вторую карточку;
- если во входном snapshot временно присутствуют два ID с одним UID, оставить только самый свежий
  живой ID; один UID запрещено одновременно показывать как два подключённых устройства;
- строить список подключённых устройств только по `sync_seen_mask`. Сохранённый `RID1`, database
  history, `last_seen` и offline TTL не разрешают оставлять строку в активном списке;
- offline TTL допустим только для журнала/истории. При переносе уже известного UID на новый ID
  старую карточку нужно убрать при обработке того же snapshot/event, не дожидаясь TTL;
- во frontend использовать UID96 как key строки и заменять состояние списка актуальным snapshot,
  иначе локальный UI-кэш может сохранить карточку со старым `device_id`.

Критерий приёмки: после изменения ID на локальном RPI портал ни в одном обновлении не показывает
две активные строки с одинаковым UID96; одна существующая карточка меняет `device_id`. Старый ID
исчезает из `sync_seen_mask` не позднее 1000 ms после прекращения его status-кадров.
...
Начиная с firmware `1.2.37` watchdog Vendor IN не вызывает
`HAL_PCD_EP_Abort()`, пока low-level `vnd_tx_busy=1`: Bulk IN в этом
состоянии принадлежит USB core и может законно ждать IN-токенов
хоста. Прежнее поведение `1.2.33` разрывало pending bulk read
после 1,2 секунды. Промежуточная `1.2.34` уже защищала активный
`DIEPCTL.EPENA=1`, но единичное чтение `EPENA=0` могло попасть в
короткое окно между аппаратным завершением и `DataIn` callback.
Поэтому с `1.2.35` аппаратный регистр вообще не используется как
разрешение на recovery. Recovery допустим только после того, как USB
class сам снял low-level busy. При disconnect/`SetInterface` class reset
выполняется явно. В `1.2.36` готовый к отправке кадр больше не
считается no-progress отказом: scheduler получает повторный kick, но кадр
не удаляется. Также уже свободный low-level endpoint не abort/не flush при
очистке только application bookkeeping. В `1.2.37` metadata кадра
публикуется до вызова `USBD_LL_Transmit`: короткий service-пакет больше не
может завершиться до появления своей meta-записи. При синхронной
ошибке submit metadata откатывается. В штатной работе `usb_force_idle` и `usb_recovery`
в COM `OPTIC` не должны расти при паузах host reader.
...
Начиная с firmware 1.2.15 активное состояние оптического датчика передаётся RPI только при
достоверной синхронизации. В режимах `MASTER`/`SLAVE` должны одновременно выполняться
`sync_signal_alive=1`, `sync_ok_visual=1`, отсутствовать конфликт локального ID и второй master.
При нарушении любого условия firmware отправляет очищенное состояние, чтобы уже начатая
реакция/звуковое сопровождение на RPI прекратились:

- в `OPTIC_STATE` очищается `payload[1] bit0`;
- в `SYNC_STATE` очищается локальный optic bit5 в `payload[13]`;
- в `SENSOR_MAP` очищается optic bit5 у всех `status_bytes[id]`;
- в совместимом `STAT` очищаются локальный/master/group optic flags, legacy optic bit и optic
  bit5 в таблице sync status.

Перед непосредственной передачей `EVT1` firmware повторно проверяет синхронизацию и очищает
optic bit также в уже поставленном в очередь пакете. Поэтому событие, сформированное до срыва
sync, не может позднее выйти из очереди как новая активная сработка. После восстановления
синхронизации актуальное состояние публикуется заново.

Биты `DetADC1/DetADC2` этим фильтром не изменяются. Физический вход, локальная индикация и
RS485-диагностика оптики продолжают работать. В намеренном автономном режиме `OFF` оптика
передаётся без sync-фильтра.
...
- 0x3B SET_LED_PATTERN: u8 pattern_id; сохраняет выбор, но сам не включает ленту
  - допустимы ID `0..15`: `OFF`, два красных/жёлтых `UP`, два красных/жёлтых `DOWN`, два красных/жёлтых `IN`, красный/жёлтый `OUT`, `UA_DEMO`
- 0x35 LED_EVENT: u8 pattern_id + u16 duration_ms; явный временный запуск
- 0x44 SET_OPTIC_REACTION_SOURCE: u8 `source_id 0..31`, `0xFF=disabled`.
  Это runtime-настройка источника оптической реакции системного WS2812; после
  reset/reconnect RSP отправляет её повторно. Роль master специального значения
  не имеет.
...
- Для firmware-индикации RSP задаёт каждой плате `0x44 source_id` либо `0xFF`,
  если системная оптическая реакция выключена. Системный WS2812 использует
  `sync_status_bytes[source_id] bit5`, а не master-флаг: выбранный соседний ID
  показывает магента/фиолетовый, собственный ID — жёлтый на slave или зелёный
  на master. Остальные датчики цвет не меняют.
- Для внешних световых эффектов нет optic-gate в STM32: ни локальный PD0, ни принятый по RS485 optic не запускают M10. Запуск выполняется только `LED_EVENT` после решения Raspberry. 200 Hz marker/TX при активном stream следует только `SET_TX_ENABLE`.
...
При reconnect RPI повторяет последнее явно выбранное пользователем значение,
но не подставляет старые `work/acquisition/detection` defaults.
...
Конфигурация `work=900000ms acquisition=5000ms detection=60000ms` устарела и не существует в новом протоколе. RPI не должен отправлять скорость периодически без необходимости: новая команда нужна только при startup/reconnect или когда действительно требуется изменить скорость.
