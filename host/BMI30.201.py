...
class ScopeWindow:
				...
				if first_len is not None:
					with self.data_lock:
						if (self.base_buf_len is None) or (self.base_buf_len != first_len):
							...
							self.base_buf_len_bytes = self.base_buf_len * 2
							# Разрешим переинициализацию слайдеров под новый размер
	...
	def _tick(self):
		...
		with self.data_lock:
			# Настроим слайдеры, когда узнаём длину кадра, либо если длина изменилась
			if self.base_buf_len is not None and not getattr(self, '_sliders_initialized', False):
